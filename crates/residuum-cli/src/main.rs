...
fn cmd_backup(source: &Path, package: &Path, json_out: bool) -> Result<(), String> {
    ...
    if json_out {
        emit_json(sjson!({
            "ok": true,
            "mode": "full_backup",
            "profile": residuum_store::BACKUP_PROFILE,
            ...
            "consistency": match report.consistency {
                residuum_store::BackupConsistency::FlushedExclusive => "flushed_exclusive",
                residuum_store::BackupConsistency::OnDiskInspect => "on_disk_inspect",
            },
        }))?;
    } else {
        println!("full_backup");
        println!("  profile: {}", residuum_store::BACKUP_PROFILE);
...
fn emit_migrate_job(
    mode: &str,
    store: &Path,
    job: &residuum_store::MigrationJob,
    json_out: bool,
) -> Result<(), String> {
    if json_out {
        emit_json(sjson!({
...
fn emit_copy_report(
    mode: &str,
    source: &Path,
    report: &residuum_store::SalvageCopyReport,
    json_out: bool,
) -> Result<(), String> {
    if json_out {
        emit_json(sjson!({
...
fn doctor_recommendations(
    salvage: &residuum_store::SalvageReport,
    summary: &UnitSummary,
    index_cache: bool,
    primary_cache: Option<&residuum_store::PrimaryCacheDiag>,
) -> Vec<String> {
    let mut out = Vec::new();
    ...
    if summary.damaged > 0 {
        out.push("damaged units present: examine with residuum-examine / SDA filters".into());
    }
    if !index_cache {
        out.push(
        ...
        use residuum_store::PrimaryCacheValidation;
        match d.validation {
