...
use residuum_sdk::{client_handshake, read_frame, write_frame, DEFAULT_MAX_FRAME_BYTES};
use std::fs;
...
fn config_validate_show_and_unsafe_reject() {
    ...
    fs::write(
        ...
        r#"{
          ...
          "format_version": 1,
          "store": { "path": "/tmp/residuum-store", "durability_default": "durable" },
          "serve": {
            "bind": "127.0.0.1:7434",
            "max_connections": 8,
...
fn serve_auth_token_required() {
    use residuum_sdk::{ConnectOptions, ErrorCode};
...
fn serve_cluster_advertises_placement_and_endpoints() {
    use residuum_sdk::{ClusterConfig, RemoteClient};
    use residuum_server::{serve_cluster_node, ServeOptions};
