...
use residuum_store::{decode_item_envelope, hex16, EventKind};
...
fn project_hole(
    source: &str,
    range: residuum_format::ByteRange,
    reason: &HoleReason,
    opts: &ProjectOptions,
) -> ExaminationUnit {
...
mod tests {
    use super::*;
    use residuum_format::{encode_frame, FrameHeader, FrameParts, START_MAGIC};

    #[test]
    fn verified_item_projects_complete_event() {
        use residuum_store::{encode_item_envelope, ItemEnvelope};
