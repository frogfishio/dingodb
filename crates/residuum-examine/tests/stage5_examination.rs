...
//! - If DingoDB can recover it, SDA can examine it (Stage 2–3 salvage outputs).
...
fn damaged_segment_sda_finds_islands_and_holes() {
    ...
    assert!(
        !islands.is_empty(),
        "if DingoDB recovers frames, SDA must examine them as verified units"
    );
    assert!(
        !holes.is_empty(),
...
fn resource_limit_never_fake_empty_complete() {
    ...
    let complete_flag = residuum_examine::eval_page(&page, "input<complete>").unwrap();
...
fn ordering_is_deterministic_across_calls() {
    ...
    let keys = |p: &residuum_examine::ExaminePage| {
        p.units
            .iter()
            .map(|u| {
                (
