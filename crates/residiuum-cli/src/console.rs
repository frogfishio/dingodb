...
/// - Otherwise, the line is treated as a RQL statement for evaluation (TBD).
pub fn run_console(_store: &Path) -> Result<(), String> {
    ...
    loop {
        ...
        // RQL-first: for now just echo until we wire the RQL engine.
        println!("(rql: TBD) {}", trimmed);
    }
...
fn print_help() {
    println!("Residiuum console v1 (mixed mode)");
    ...
    println!("RQL-first: other lines are treated as RQL (v1 syntax TBD).\n");
}
