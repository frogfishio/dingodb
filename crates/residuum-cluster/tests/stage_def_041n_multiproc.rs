...
use residuum_cluster::{MultiprocHistory, MULTIPROC_PROFILE};
...
fn child_bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_residuum-cluster-multiproc-child"))
}
...
fn multiproc_profile_stable() {
    assert_eq!(MULTIPROC_PROFILE, "residuum-cluster-multiproc-v1");
}
