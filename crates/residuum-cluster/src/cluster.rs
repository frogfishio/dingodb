...
//!
//! Each node is an ordinary [`residuum_store::Store`]. Writes in
...
use residuum_store::{DurabilityMode, SalvageReport, Store};
...
pub struct Cluster {
    ...
    token_keyring: residuum_store::ContinuationKeyring,
}
...
impl Cluster {
    ...
    pub fn create(cfg: ClusterConfig) -> Result<Self, ClusterError> {
        ...
        let token_keyring = residuum_store::ContinuationKeyring::mint_new().map_err(|e| {
    ...
    pub fn open(root: impl AsRef<Path>) -> Result<Self, ClusterError> {
        ...
        let token_keyring = residuum_store::ContinuationKeyring::load_or_mint_cluster(&root)
            .map_err(|e| {
    ...
    pub fn continuation_keyring(&self) -> &residuum_store::ContinuationKeyring {
        &self.token_keyring
    }
        ...
        for node in &participants {
            ...
            for (subj_bytes, _body) in store.live_entries() {
                ...
                let live = match store.get(subject) {
                    Ok(Some(v)) => v,
                    Ok(None) => continue,
                    Err(residuum_store::StoreError::PayloadPartial) => continue,
        ...
        for (subj_bytes, body) in store.live_entries() {
            ...
            *docs_scanned += 1;
            ...
            match store.get(subject) {
                ...
                Ok(None) => {}
                Err(residuum_store::StoreError::PayloadPartial) => {
                    let _ = body;
                }
                Err(e) => return Err(e.into()),
            }
    ...
    pub fn subject_history(
        &mut self,
        subject: &str,
    ) -> Result<residuum_store::SubjectHistory, ClusterError> {
        let partition = self.partition_for_subject(subject);
        ...
        for subject in subject_list {
            ...
            for n in &online_replicas {
                ...
                let view = match store.get(&subject) {
                    ...
                    Err(residuum_store::StoreError::PayloadPartial)
                    | Err(residuum_store::StoreError::PayloadConflict) => NodeSubjectView {
                        node: *n,
