...
//! remain in ordinary `residuum-store` segments and are independently salvageable.
...
impl RaftPeerStore {
    ...
    pub fn rewrite_log(&self, entries: &[LogEntry]) -> Result<(), ClusterError> {
        ...
        residuum_store::write_atomic(&path, &buf)?;
        Ok(())
    }
    ...
    pub fn install_snapshot(
        ...
        let blob_path = self.root.join(SNAPSHOT_BLOB_FILE);
        residuum_store::write_atomic(&blob_path, blob)?;
...
fn write_checksum_doc<T: Serialize>(path: &Path, body: &T) -> Result<(), ClusterError> {
    ...
    residuum_store::write_atomic_keep_previous(path, &bytes)?;
    Ok(())
}
...
fn read_checksum_doc<T: for<'de> Deserialize<'de>>(
    ...
    let prev = residuum_store::previous_path(path);
    if prev.is_file() {
