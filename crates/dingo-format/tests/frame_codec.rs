...
use dingo_format::{
    ...
    SafetyLimits, END_MAGIC, FRAME_PREFIX_LEN, FRAME_SUFFIX_LEN, START_MAGIC, WIRE_MAJOR,
    WIRE_MINOR,
};
...
fn prefix_and_suffix_field_boundaries() {
    let envelope = vec![0u8; 0];
    let body = vec![0u8; 0];
    let encoded = encode_frame(&parts(FrameKind::Padding, &envelope, &body)).unwrap();
    assert_eq!(encoded.len(), FRAME_PREFIX_LEN + FRAME_SUFFIX_LEN);
...
fn max_small_envelope_and_body_roundtrip() {
    let envelope = (0u8..64).collect::<Vec<_>>();
...
fn unknown_extension_kind_is_opaque_but_verified() {
    let mut p = parts(FrameKind::ItemEvent, b"", b"ext");
...
fn one_byte_body_corruption_rejects() {
    let mut encoded = encode_frame(&parts(FrameKind::ItemEvent, b"e", b"BODYDATA")).unwrap();
    let body_at = FRAME_PREFIX_LEN + 1;
...
fn one_byte_envelope_corruption_rejects_prefix_crc() {
    let mut encoded = encode_frame(&parts(FrameKind::ItemEvent, b"ENV", b"body")).unwrap();
...
fn one_byte_truncation_positions_reject() {
    let encoded = encode_frame(&parts(FrameKind::ItemEvent, b"xx", b"yyy")).unwrap();
...
fn body_hash_matches_blake3() {
    ...
    let encoded = encode_frame(&parts(FrameKind::PayloadChunk, b"", body)).unwrap();
    let decoded = decode_frame(&encoded, SafetyLimits::default()).unwrap();
...
fn corrupt_length_fields_fail_closed() {
    let mut encoded = encode_frame(&parts(FrameKind::ItemEvent, b"ab", b"cd")).unwrap();
