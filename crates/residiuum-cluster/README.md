...
**Cluster federation** for Residiuum: partition routing, coverage records,
placement directory, multi-node stores built on ordinary
[`residiuum-store`](https://crates.io/crates/residiuum-store) nodes, per-partition
...
[`residiuum-sdk`](https://crates.io/crates/residiuum-sdk) (`features = ["cluster"]`).
...
| You want… | Use |
|-----------|-----|
| Collection API on a multi-node cluster | [`residiuum-sdk`](https://crates.io/crates/residiuum-sdk) with `cluster` |
| Partition, Raft, coverage, rebalance APIs | **`residiuum-cluster`** (this crate) |
| Single-node store only | [`residiuum-store`](https://crates.io/crates/residiuum-store) |
| TCP serve of a cluster node | [`residiuum-server`](https://crates.io/crates/residiuum-server) / `residiuum serve-cluster` |

## Install

```toml
[dependencies]
residiuum-cluster = "0.1"
```

Or: `cargo add residiuum-cluster`
...
> this crate; only `residiuum-sdk` with `features = ["cluster"]` pulls it in.
...
```rust
use residiuum_cluster::{Cluster, ClusterConfig, ReadMode, ScanOptions};
...
```rust
use residiuum_cluster::{Cluster, ClusterConfig, NodeId, PartitionId, RebalancePhase};
...
cluster-root/
  ...
  nodes/
    node-0/             # ordinary residiuum-store directory
    node-1/
    ...
```
...
unreadable. Open any node path with `residiuum_store::Store` / `salvage` as usual.
...
- In-process quorum remains `Residiuum::open_cluster` / `Cluster::create`.
...
| Crate | License | Role |
|-------|---------|------|
| [`residiuum-store`](https://crates.io/crates/residiuum-store) | MPL-2.0 | Per-node store |
| [`residiuum-sdk`](https://crates.io/crates/residiuum-sdk) | MPL-2.0 | Collection API + `cluster` feature |
| [`residiuum-server`](https://crates.io/crates/residiuum-server) | AGPL-3.0-or-later | Network serve + Raft RPC glue |
| [`residiuum-cli`](https://crates.io/crates/residiuum-cli) | AGPL-3.0-or-later | `residiuum serve-cluster` |
