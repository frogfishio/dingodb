...
**SDA examination host** for Residiuum: map recovered frames, items, and holes to
...
[`residiuum-sda`](https://crates.io/crates/residiuum-sda); this crate is the host that
...
| You want… | Use |
|-----------|-----|
| Pure SDA over arbitrary JSON | [`residiuum-sda`](https://crates.io/crates/residiuum-sda) |
| Examine a Residiuum store / salvage stream | **`residiuum-examine`** (this crate) |
| CLI health report | `residiuum doctor` ([`residiuum-cli`](https://crates.io/crates/residiuum-cli)) |

## Install

```toml
[dependencies]
residiuum-examine = "0.1"
residiuum-store = "0.1"
```

Or: `cargo add residiuum-examine`
...
host before evaluation. The `residiuum doctor` CLI uses this crate for recovery
unit summaries.

## Quick example

```rust
use residiuum_examine::{examine_store, filter_units, ExamineLimits};
...
| Crate | License | Role |
|-------|---------|------|
| [`residiuum-sda`](https://crates.io/crates/residiuum-sda) | MIT | Pure SDA language runtime |
| [`residiuum-store`](https://crates.io/crates/residiuum-store) | MPL-2.0 | Store salvage sources |
| [`residiuum-format`](https://crates.io/crates/residiuum-format) | MIT | Frame verification under salvage |
| [`residiuum-cli`](https://crates.io/crates/residiuum-cli) | AGPL-3.0-or-later | `residiuum doctor` |
