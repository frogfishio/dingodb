...
use dingo_sdk::plan_v1::{
    CollectionBindings, OrderDir, PlanBuilder, RqlPlanV1, PLAN_ENCODING_PROFILE, PLAN_HASH_DOMAIN,
    PLAN_PROFILE,
};
use dingo_sdk::predicate::{field, param, Predicate};
use dingo_sdk::{CoveragePolicy, PREDICATE_PROFILE};
use dingo_heap::CollectionId;
...
fn app4_predicate_totality_model() {
    ...
    let p_eq = Predicate::field_eq_param(
        dingo_sdk::PredPath::parse_dotted("status").unwrap(),
        "status",
    );
...
fn app4_name_binding_required() {
    let err = PlanBuilder::from_source("missing")
        ...
        .unwrap_err();
    assert!(err.to_string().contains("unknown collection") || matches!(err, dingo_sdk::Error::QueryInvalid(_)));
}
