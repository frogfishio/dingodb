...
fn tcp_request_vote_and_append_across_three_peers() {
    ...
    let mut req = dingo_sdk::RpcRequest {
        id: 0,
    ...
    match client1.call_rpc(req) {
        ...
        Err(e) => {
            ...
            assert!(
                e.code() == dingo_sdk::ErrorCode::ConsistencyViolation
...
fn unauthenticated_raft_rpc_rejected_when_token_required() {
    ...
    match client {
        Err(e) => {
            let c = e.code();
            assert!(
                c == dingo_sdk::ErrorCode::AuthenticationFailed
                    || c == dingo_sdk::ErrorCode::Internal
                    || format!("{e}").contains("auth"),
                "expected auth failure, got {e}"
            );
        }
        Ok(mut c) => {
            let req = dingo_sdk::RpcRequest {
                id: 1,
                op: "raft_read_index".into(),
                collection: None,
                key: None,
