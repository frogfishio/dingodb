...
use dingo_store::{DurabilityMode, Store, WriteReceipt as StoreWriteReceipt};
...
impl RaftServerState {
    ...
    pub fn ensure_leader(&mut self, partition: u32) -> Result<(NodeId, dingo_cluster::Term), Error> {
        if self.is_leader(partition) {
            let t = self.term(partition);
            return Ok((self.local, dingo_cluster::Term(t)));
        }
...
fn map_elect_err(partition: u32, e: dingo_cluster::ElectError) -> Error {
    match e {
        dingo_cluster::ElectError::NoQuorum { votes, need } => Error::StaleRoute {
            partition,
            message: format!(
                "election no quorum votes={votes} need={need} (code=not_leader)"
            ),
        },
        dingo_cluster::ElectError::NotAVoter => Error::StaleRoute {
            partition,
            message: "not a voter for partition (code=not_leader)".into(),
        },
        dingo_cluster::ElectError::CandidateOffline => Error::PartitionUnavailable {
            partition,
