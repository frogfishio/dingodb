...
use dingo_store::{DurabilityMode, EventKind, PayloadResult, Store, DEFAULT_CHUNK_SIZE};
use std::fs;
...
fn chunked_put_get_and_partial_missing_middle() {
    ...
    let raw = store
        ...
        .map(|(_, b)| b.to_vec())
        .unwrap();
    assert!(dingo_store::is_chunk_manifest(&raw));
...
fn secondary_index_create_lookup_delete() {
    use dingo_store::{IndexState, SecondaryIndex};
