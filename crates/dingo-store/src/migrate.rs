...
/// `dingo-client::PROTOCOL_MAJOR`; kept here so the store crate does not
/// depend on the client).
pub const PROTOCOL_MAJOR_DECLARED: u16 = 1;

/// Declared network protocol minor (mirrors `dingo-client::PROTOCOL_MINOR`).
pub const PROTOCOL_MINOR_DECLARED: u16 = 0;

/// Declared RPC profile tag (mirrors `dingo-client::PROTOCOL_PROFILE`).
pub const PROTOCOL_PROFILE_DECLARED: &str = "dingo-rpc-v1";

/// Draft RPC interoperability label (mirrors `dingo-client::RPC_WIRE_LABEL`).
    ...
    wire_compat_matrix()
        ...
        .map(|e| WireMatrixRow {
            ...
            status: match e.status {
                dingo_format::WireSupportStatus::Current => "current".into(),
                dingo_format::WireSupportStatus::ReadOnly => "read_only".into(),
                dingo_format::WireSupportStatus::Deprecated => "deprecated".into(),
                dingo_format::WireSupportStatus::Unsupported => "unsupported".into(),
            },
        })
        .collect()
}
