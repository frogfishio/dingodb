...
fn seal_pending_bytes(
    ...
    for region in &report.regions {
        match region {
            ...
            dingo_format::ScanRegion::Hole { .. } => break,
        }
    }
    ...
    .map_err(|e| StoreError::CorruptMeta(match e {
        dingo_format::SegmentError::MissingDescriptor => "pending missing descriptor",
        dingo_format::SegmentError::AlreadySealed => "pending already sealed",
...
mod tests {
    use super::*;
    use dingo_format::{ActiveSegment, FrameKind, SegmentId};
    ...
    fn seal_pending_preserves_prefix_bytes() {
        ...
        let (_h, _e, body, _, _) =
            dingo_format::verify_frame_at(&sealed[off as usize..], SafetyLimits::default())
                .unwrap();
