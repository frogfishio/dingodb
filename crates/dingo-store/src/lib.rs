...
pub use heap::{
    active_snapshot, build_backup_manifest, create_object, decode_purge_receipt,
    ...
    labelled_unit_readable, load_identity_tombstone, load_staged_genesis,
    old_deployment_credential_invalid, publish_staged_genesis, rebuild_and_persist_all_catalogs,
    rebuild_heap_entry_from_chain, rebuild_object_entry_from_chain,
    ...
    require_admit, restore_payload_to_new_heap, retire_heap, retire_object, stage_heap_genesis,
    staging_is_non_discoverable, try_load_collections_catalog, try_load_heap_catalog,
    try_load_streams_catalog, verify_purge_receipt, wipe_heap_object_media,
    write_identity_tombstone,
    AdminReceipt, CutoverGate, DataKeyDestructionReceipt, DataKeyHandle, DataKeyProvider,
    DisasterRecoveryCeremony, DisasterRecoveryPackage, DisasterRecoveryTakeoverResult,
    HeapBackupManifest, HeapCatalogEntry, HeapLifecycle, HeapMetaLayout, HeapMigrationJob,
    HeapRetentionPolicy, HeapStore, HsmBackendKind, HsmCapabilities, HsmDataKeyConfig,
    HsmDataKeyProvider, IdentityTombstone, IncompletePurgeResult, InProcessDataKeyProvider,
    InventoryFrame, InventorySegment, MaintenanceStore, MediaDomain, MigrationPhase,
    MigrationStateV1, MixedHeapSalvageClass, ObjectCatalogEntry, ObjectKind, PayloadOnlyRestore,
    PurgeCoverageUnit, PurgePlan, PurgeReceipt, RecoveryStore, ReplicaStore, RetentionScheduler,
    SourceInventory, StagedGenesis, StoreHost, TombstoneKind,
