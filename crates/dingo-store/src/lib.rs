...
pub use heap::{
    ...
    encode_purge_receipt, heap_binding_envelope, heap_label_envelope, labelled_unit_readable,
    load_identity_tombstone, load_staged_genesis, old_deployment_credential_invalid,
    publish_staged_genesis, rebuild_and_persist_all_catalogs, rebuild_heap_entry_from_chain,
    rebuild_object_entry_from_chain, refuse_access_from_payload_restore,
    refuse_clear_tombstone_via_payload_restore, refuse_retain_id_without_ceremony, rename_heap,
    rename_object, require_admit, restore_payload_to_new_heap, retire_heap, retire_object,
    stage_heap_genesis, staging_is_non_discoverable, try_load_collections_catalog,
    try_load_heap_catalog, try_load_streams_catalog, verify_purge_receipt, write_identity_tombstone,
    AdminReceipt, CutoverGate, DataKeyDestructionReceipt, DataKeyHandle, DisasterRecoveryCeremony,
    ...
    IdentityTombstone, IncompletePurgeResult, InventoryFrame, InventorySegment, MaintenanceStore,
    MediaDomain, MigrationPhase, MigrationStateV1, ObjectCatalogEntry, ObjectKind,
    PayloadOnlyRestore, PurgeCoverageUnit, PurgePlan, PurgeReceipt, RecoveryStore, ReplicaStore,
    RetentionScheduler, SourceInventory, StagedGenesis, StoreHost, TombstoneKind,
