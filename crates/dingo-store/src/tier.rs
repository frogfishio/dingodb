...
pub fn classify_segment_bytes(bytes: &[u8]) -> FormatClassification {
    use dingo_format::{scan_forward, FrameVerifyError, SafetyLimits, WIRE_MAJOR};
    ...
    for region in &report.regions {
        match region {
            dingo_format::ScanRegion::VerifiedFrame { frame, .. } => {
            ...
            dingo_format::ScanRegion::Hole { reason, .. } => {
                if let dingo_format::HoleReason::CorruptCandidate {
