use residiuum_sda::{run, run_with_input_binding, SdaError};
...
/// [`residiuum_sda::CONFORMANCE_CORPUS_TAG`].
mod section_14_must_lock {
    ...
    fn frozen_corpus_tag_is_stable() {
        assert_eq!(residiuum_sda::CONFORMANCE_CORPUS_TAG, "sda-standalone-v1.0");
        assert_eq!(
            residiuum_sda::SdaRuntime::conformance_corpus_tag(),
            "sda-standalone-v1.0"
        );
        let version_file = include_str!("sda/VERSION").trim();
        assert_eq!(version_file, residiuum_sda::CONFORMANCE_CORPUS_TAG);
    }
}
