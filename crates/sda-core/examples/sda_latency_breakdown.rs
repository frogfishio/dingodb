...
//!
//! Run: `cargo run -p sda-lib --release --example sda_latency_breakdown`
//!
...
use sda_lib::{from_json, to_json, Program};
...
fn main() {
    ...
    for case in cases() {
        ...
        for _ in 0..warmup {
            ...
            let _ = sda_lib::run(case.source, case.input.clone()).expect("run warmup");
        }
        ...
        let full_samples = bench_durations(n, || {
            let _ = sda_lib::run(case.source, case.input.clone()).expect("run");
        });
