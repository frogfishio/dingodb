...
use residuum_sdk::{client_connect, TlsClientOptions, TlsServerOptions};
use residuum_server::{
...
fn issue_localhost_tls(dir: &std::path::Path) -> (PathBuf, PathBuf, PathBuf) {
    ...
    ca_params
        .distinguished_name
        .push(DnType::CommonName, "residuum-heap-test-ca");
...
fn live_tls_accept_loop_heap_key_ping_without_token() {
    ...
    assert!(
        audit.snapshot().iter().any(|e| matches!(
            e,
            residuum_server::HeapAuthAuditEvent::Welcome { .. }
        )),
