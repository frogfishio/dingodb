...
use residuum_client::{b64u_encode, HeapAuth, HeapReject, FEATURE_HEAP_KEY_V1};
use residuum_format::{encode_deterministic_uint_map, CborValue};
use residuum_heap::{
...
fn successful_auth_then_proof_replay_fails() {
    ...
    let cap = match first {
        HeapAuthOutcome::Welcome { welcome, cap } => {
            ...
            assert_eq!(welcome.heap_profile, "residuum-heap-v1");
            cap
        }
...
fn hot_path_modules_do_not_reference_authority_store() {
    ...
    for name in [
        ...
        assert!(
            !src.contains("residuum_authority") && !src.contains("MasterAuthorityStore"),
...
fn qualified_session_welcome_and_reject_are_indistinguishable_on_wire() {
    ...
    residuum_client::write_frame(&mut server_in, &auth_body).unwrap();
    ...
    let mut server_in = Vec::new();
    write_json_frame(&mut server_in, &hello).unwrap();
    residuum_client::write_frame(&mut server_in, &bad_auth).unwrap();
    let mut reader = Cursor::new(server_in);
    let mut writer = Vec::new();
