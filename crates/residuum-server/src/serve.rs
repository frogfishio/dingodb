...
//!
//! Extracted from the former monolithic `residuum-sdk` remote module.
...
use residuum_cluster::{DEFAULT_VIRTUAL_PARTITIONS, HASH_PROFILE_BLAKE3_MOD};
use residuum_sdk::{
...
use residuum_store::{DurabilityMode, PayloadResult, Store, WriteReceipt as StoreWriteReceipt};
...
fn prepare_mutation_dedup(
    ...
    let content_hash = residuum_store::content_identity(op, collection, key, payload);
...
/// [`residuum_store::StoreHost`] so §32.4 data ops stay capability-gated.
fn qualified_connection_shared(
    stream: IoStream,
    options: &ServeOptions,
    store: &std::sync::Arc<std::sync::Mutex<residuum_store::Store>>,
    ...
    let host = residuum_store::StoreHost::from_shared(std::sync::Arc::clone(store), store_path);
...
fn server_handshake_buffered(reader: &mut BufReader<IoStream>) -> Result<NegotiatedSession, Error> {
    ...
    // Temporarily implement inline using the same logic as residuum_sdk::server_handshake.
    let payload = match read_frame_or_detect_legacy(reader, residuum_sdk::HANDSHAKE_MAX_FRAME_BYTES)? {
        Some(p) => p,
        None => {
    ...
    let hello = match residuum_sdk::parse_handshake(&payload) {
        Ok(h) => h,
        Err(e) => {
            let _ = write_json_frame(
                reader.get_mut(),
                &residuum_sdk::Handshake::reject("protocol_violation", e.to_string()),
            );
            return Err(e);
        }
    };
    if hello.msg != residuum_sdk::HandshakeMsg::Hello {
        let msg = format!("expected hello, got {:?}", hello.msg);
        let _ = write_json_frame(
            reader.get_mut(),
            &residuum_sdk::Handshake::reject("protocol_violation", &msg),
        );
        return Err(Error::ProtocolViolation(msg));
    }
    if hello.v != residuum_sdk::PROTOCOL_MAJOR
        && hello.protocol_major.unwrap_or(hello.v) != residuum_sdk::PROTOCOL_MAJOR
    {
        let msg = format!(
            "unsupported protocol major {} (server speaks {})",
            hello.v,
            residuum_sdk::PROTOCOL_MAJOR
        );
        let _ = write_json_frame(
            reader.get_mut(),
            &residuum_sdk::Handshake::reject("protocol_version_unsupported", &msg),
        );
        return Err(Error::ProtocolViolation(msg));
    }
    let features = match residuum_sdk::negotiate_features(hello.features.as_deref().unwrap_or(&[])) {
        Ok(f) => f,
        Err(e) => {
            let _ = write_json_frame(
                reader.get_mut(),
    ...
    let max_frame = residuum_sdk::negotiate_max_frame(hello.max_frame);
    write_json_frame(
        reader.get_mut(),
        &residuum_sdk::Handshake::welcome(max_frame, features.clone()),
    )?;
    Ok(NegotiatedSession {
        max_frame,
        features,
        protocol_major: residuum_sdk::PROTOCOL_MAJOR,
        protocol_minor: residuum_sdk::PROTOCOL_MINOR,
    })
}
...
pub fn serve_cluster_node(
    ...
    let meta = residuum_cluster::ClusterMeta::load(root)
    ...
    let endpoints = residuum_cluster::upsert_endpoint(root, node_index, bind)
        .map_err(|e| Error::Internal(format!("endpoints.json: {e}")))?;

    let directory = residuum_cluster::PartitionDirectory::load(root)
    ...
    let store_path = residuum_cluster::node_store_path(root, node_index);
        ...
        match raft_server::shared_raft_state(root, node_index, opts.auth_token.clone()) {
            Ok(state) => {
                ...
                if let Ok(mut g) = state.lock() {
                    if let Ok(eps) = residuum_cluster::load_endpoints(root) {
                        g.set_endpoints(eps);
                    }
                }
...
pub fn handle_connection_shared(
    ...
    let max_frame = if options.diagnostic_line_protocol {
        residuum_sdk::host_limits().max_rpc_line_bytes
    } else {
...
fn read_rpc_request(
    ...
    if options.diagnostic_line_protocol {
        ...
        if let Err(e) = residuum_sdk::check_rpc_line_len(line.len(), &residuum_sdk::host_limits()) {
        ...
        let bytes = match residuum_sdk::read_frame(reader, max_frame) {
            Ok(Some(b)) => b,
            Ok(None) => return Ok(None),
            Err(Error::DeadlineExceeded(_)) => {
                return Err(ReadRpc::Idle);
            }
            Err(e) if e.code() == residuum_sdk::ErrorCode::ResourceLimit => {
                let _ = write_rpc_response(
                    reader,
                    options,
                    RpcResponse {
...
fn partition_for_subject(options: &ServeOptions, subject: &str) -> u32 {
    use residuum_cluster::PartitionMap;
...
fn mutation_via_raft_or_store(
    ...
    is_delete: bool,
    operation_id_hex: Option<&str>,
) -> Result<residuum_store::WriteReceipt, Error> {
    ...
    let command = if is_delete {
        residuum_cluster::LogCommand::Delete {
            subject: subject.to_string(),
        }
    } else {
        residuum_cluster::LogCommand::Put {
            subject: subject.to_string(),
            value: body.to_vec(),
        }
    };
...
fn dispatch(
    ...
    match req.op.as_str() {
        ...
        "put" => {
            ...
            let limits = residuum_sdk::host_limits();
            residuum_sdk::check_json_depth(json, &limits)?;
            let mode =
            ...
            residuum_sdk::check_payload_len(body.len(), &limits)?;
        ...
        "put_bytes" => {
            ...
            let limits = residuum_sdk::host_limits();
            residuum_sdk::check_payload_len(bytes.len().saturating_add(1), &limits)?;
            let mode =
                parse_durability(req.durability.as_deref()).unwrap_or(DurabilityMode::Durable);
            let subject = encode_subject(coll, key)?;
            let subject_str = str::from_utf8(&subject).expect("stage 4 subject is UTF-8");
