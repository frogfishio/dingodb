...
fn dispatch_history(id: u64, req: &HeapRpcRequest, ctx: HeapDataCtx<'_>) -> HeapDispatchResult {
    ...
    for ev in hist.events {
        let kind = match ev.kind {
            residuum_store::EventKind::Put => "put",
            residuum_store::EventKind::Delete => "delete",
        };
        ...
        if ev.kind == residuum_store::EventKind::Put && ev.body.first() == Some(&0x01) {
        ...
        Ok(Some(body)) => {
            if bytes_mode {
                ...
                if body.first() == Some(&0x02) {
                    let b64 = residuum_client::b64u_encode(&body[1..]);
                    ok_id(
                        id,
...
fn index_meta_json(idx: &residuum_store::SecondaryIndex) -> Value {
    serde_json::json!({
        "name": idx.meta.name,
