...
pub fn run_qualified_handshake<R: Read, W: Write>(
    ...
    let hello = match parse_handshake(&hello_payload) {
        ...
        Err(e) => {
            let _ = write_json_frame(
                writer,
                &residuum_client::Handshake::reject("protocol_violation", e.to_string()),
            );
    ...
    if hello.msg != HandshakeMsg::Hello {
        ...
        let _ = write_json_frame(
            writer,
            &residuum_client::Handshake::reject("protocol_violation", &msg),
        );
    ...
    if hello.v != PROTOCOL_MAJOR && hello.protocol_major.unwrap_or(hello.v) != PROTOCOL_MAJOR {
        ...
        let _ = write_json_frame(
            writer,
            &residuum_client::Handshake::reject("protocol_version_unsupported", &msg),
        );
        return Err(Error::ProtocolViolation(msg));
    }
    ...
    let max_frame = residuum_client::negotiate_max_frame(hello.max_frame);
...
/// are dispatched through a capability-gated [`residuum_store::HeapStore`].
