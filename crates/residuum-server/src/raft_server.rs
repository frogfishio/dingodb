...
use residuum_sdk::DirectorySnapshot;
use residuum_sdk::Error;
use residuum_sdk::{ConnectOptions, RemoteClient, RpcRequest};
use residuum_cluster::raft_rpc::{
...
use residuum_store::{DurabilityMode, Store, WriteReceipt as StoreWriteReceipt};
...
pub use residuum_cluster::RAFT_RPC_PROFILE as SDK_RAFT_RPC_PROFILE;

/// Profile tag for data-plane cluster commitment over network Raft (DEF-037).
pub const CLUSTER_COMMIT_PROFILE: &str = "residuum-cluster-commit-v1";
...
impl RaftServerState {
    ...
    pub fn open(
        ...
        let endpoints = residuum_cluster::load_endpoints(cluster_root).unwrap_or_default();
    ...
    pub fn reload_endpoints(&mut self, cluster_root: &Path) {
        if let Ok(eps) = residuum_cluster::load_endpoints(cluster_root) {
            self.endpoints = eps;
        }
    }
    ...
    pub fn campaign(&mut self, partition: u32) -> Result<(NodeId, residuum_cluster::Term), Error> {
    ...
    pub fn ensure_leader(&mut self, partition: u32) -> Result<(NodeId, residuum_cluster::Term), Error> {
        if self.is_leader(partition) {
            let t = self.term(partition);
            return Ok((self.local, residuum_cluster::Term(t)));
        }
...
fn raft_err(e: residuum_cluster::RaftRpcError) -> Error {
    match e {
        residuum_cluster::RaftRpcError::Unauthorized(s) => Error::AuthenticationFailed(s),
        residuum_cluster::RaftRpcError::Fenced(s) => Error::ConsistencyViolation(s),
        residuum_cluster::RaftRpcError::Protocol(s) => Error::ProtocolViolation(s),
...
fn map_elect_err(partition: u32, e: residuum_cluster::ElectError) -> Error {
    match e {
        residuum_cluster::ElectError::NoQuorum { votes, need } => Error::StaleRoute {
            partition,
            message: format!(
                "election no quorum votes={votes} need={need} (code=not_leader)"
            ),
        },
        residuum_cluster::ElectError::NotAVoter => Error::StaleRoute {
            partition,
            message: "not a voter for partition (code=not_leader)".into(),
        },
        residuum_cluster::ElectError::CandidateOffline => Error::PartitionUnavailable {
            partition,
...
impl RaftTransport for TcpRaftTransport {
    ...
    ) -> Result<RequestVoteResponse, residuum_cluster::RaftRpcError> {
        let body = serde_json::to_value(req).map_err(|e| {
            residuum_cluster::RaftRpcError::Protocol(format!("encode request_vote: {e}"))
        })?;
        let v = self
            .call_json(to, "raft_request_vote", &body)
            .map_err(|e| residuum_cluster::RaftRpcError::Unavailable(e.to_string()))?;
        serde_json::from_value(v)
            .map_err(|e| residuum_cluster::RaftRpcError::Protocol(format!("decode: {e}")))
    }
    ...
    ) -> Result<AppendEntriesResponse, residuum_cluster::RaftRpcError> {
        let body = serde_json::to_value(req).map_err(|e| {
            residuum_cluster::RaftRpcError::Protocol(format!("encode append_entries: {e}"))
        })?;
        let v = self
    ...
    ) -> Result<InstallSnapshotResponse, residuum_cluster::RaftRpcError> {
        let body = serde_json::to_value(req).map_err(|e| {
            residuum_cluster::RaftRpcError::Protocol(format!("encode install_snapshot: {e}"))
        })?;
        let v = self
    ...
    ) -> Result<ReadIndexResponse, residuum_cluster::RaftRpcError> {
        let body = serde_json::to_value(req)
            .map_err(|e| residuum_cluster::RaftRpcError::Protocol(format!("encode read_index: {e}")))?;
        let v = self
