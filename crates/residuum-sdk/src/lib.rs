...
//! `cluster` feature depends on AGPL `residuum-cluster` (in-process multi-node).
//! Network **serve** lives in AGPL `residuum-server`. Wire framing lives in MIT
//! `residuum-client` (re-exported here for compatibility).
//!
...
// --- MIT residuum-client protocol (constants + types re-exported; IO maps errors) ---
pub use residuum_client::{
...
/// Encode a length-prefixed frame (see `residuum-client`).
pub fn encode_frame(payload: &[u8]) -> Result<Vec<u8>, Error> {
    residuum_client::encode_frame(payload).map_err(Error::from)
}

/// Write one framed payload.
pub fn write_frame<W: std::io::Write>(w: &mut W, payload: &[u8]) -> Result<(), Error> {
    residuum_client::write_frame(w, payload).map_err(Error::from)
}
...
pub fn write_json_frame<W: std::io::Write, T: serde::Serialize>(
    ...
    residuum_client::write_json_frame(w, value).map_err(Error::from)
}
...
pub fn read_frame<R: std::io::Read>(
    ...
    residuum_client::read_frame(r, max_frame).map_err(Error::from)
}
...
pub fn server_handshake<R: std::io::Read, W: std::io::Write>(
    ...
    residuum_client::server_handshake(reader, writer).map_err(Error::from)
}
...
pub fn client_handshake<R: std::io::Read, W: std::io::Write>(
    ...
    residuum_client::client_handshake(reader, writer).map_err(Error::from)
}
...
pub use residuum_store::DurabilityMode;
/// Re-export index lifecycle states (DX_SPEC §8.2).
pub use residuum_store::IndexState;
/// Re-export chunked payload completeness (FORMAT_SPEC §8).
pub use residuum_store::PayloadResult;
