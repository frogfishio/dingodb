...
impl Residuum {
    ...
    ///
    /// Requires the `cluster` feature (AGPL `residuum-cluster`).
    ///
    ...
    pub fn compact_live(&mut self) -> Result<residuum_store::CompactReport, Error> {
        match &mut self.backend {
    ...
    pub fn checkpoint(
        &self,
        coverage: &str,
    ) -> Result<(residuum_store::CheckpointMeta, PathBuf), Error> {
        match &self.backend {
