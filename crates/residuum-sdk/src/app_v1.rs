...
use residuum_heap::{CollectionId, HeapId};
use serde::Serialize;
...
/// DQL Application Core source profile.
pub const DQL_APP_CORE_PROFILE: &str = "dql-app-core-v1";

/// Canonical logical plan profile.
pub const DQL_PLAN_PROFILE: &str = "dql-plan-v1";
...
/// Options for `dql` / `explain_dql` / page execution.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl CollectionClient {
    ...
    /// DQL Application Core execution (APP-5…APP-7).
    pub fn dql(
        &mut self,
        _source: &str,
        ...
        Err(Error::Internal(
            "APP-0 contract surface: dql activates in APP-5…APP-7".into(),
        ))
    }

    /// DQL explain (APP-5…APP-7).
    pub fn explain_dql(
        &mut self,
        _source: &str,
        _parameters: &Parameters,
        ...
        Err(Error::Internal(
            "APP-0 contract surface: explain_dql activates in APP-5…APP-7".into(),
        ))
    }
}
