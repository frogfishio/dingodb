...
use residuum_store::IndexState;
#[cfg(feature = "legacy-flat-sdk")]
use residuum_store::{SecondaryIndex, Store};
...
impl IndexInfo {
    ...
    pub fn from_store(idx: &SecondaryIndex) -> Self {
        Self {
            ...
            build_id_hex: residuum_store::hex16(&idx.meta.build_id),
        }
    }
}
...
fn create_index_on_store_inner(
    ...
    let mut idx = if let Some(prior) = resume {
        ...
        let build_id = residuum_store::random_id().map_err(Error::from)?;
        ...
        residuum_store::hit_failpoint(FP_INDEX_BUILD_AFTER_PLAN)?;
        fresh
    };
    ...
    match run_index_build(store, &mut idx) {
        ...
        Err(e) => {
            ...
            if !matches!(
                &e,
                Error::Store(residuum_store::StoreError::Failpoint(_))
            ) {
...
fn run_index_build(store: &mut Store, idx: &mut SecondaryIndex) -> Result<(), Error> {
    ...
    residuum_store::hit_failpoint(FP_INDEX_BUILD_BEFORE_READY)?;
    ...
    loop {
        ...
        for (subject, body) in page.entries {
            ...
            if since_checkpoint >= BUILD_CHECKPOINT_EVERY {
                store.write_secondary_index(idx)?;
                residuum_store::hit_failpoint(FP_INDEX_BUILD_MID)?;
                since_checkpoint = 0;
            }
        }
