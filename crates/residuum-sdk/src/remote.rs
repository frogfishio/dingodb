...
//! `residuum_server::ServeOptions` — not on the collection API (DX_SPEC §4.2).
...
pub struct ConnectOptions {
    ...
    /// `residuum_server::ServeOptions::diagnostic_line_protocol`. Production clients leave
...
impl RemoteClient {
    ...
    ///
    /// Used by [`residuum_server::TcpRaftTransport`] so peer RPCs do not need
