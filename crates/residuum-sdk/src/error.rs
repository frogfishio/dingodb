...
impl From<residuum_client::Error> for Error {
    fn from(e: residuum_client::Error) -> Self {
        match e {
            residuum_client::Error::ProtocolViolation(s) => Self::ProtocolViolation(s),
            residuum_client::Error::ResourceLimit(s) => Self::ResourceLimit(s),
            residuum_client::Error::AuthenticationFailed(s) => Self::AuthenticationFailed(s),
            residuum_client::Error::DeadlineExceeded(s) => Self::DeadlineExceeded(s),
            residuum_client::Error::Remote { code, message } => Self::Remote { code, message },
            residuum_client::Error::Internal(s) => Self::Internal(s),
            residuum_client::Error::Io(e) => Self::from_io(e),
        }
    }
}