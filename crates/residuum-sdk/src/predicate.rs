...
//!
//! Normative: [RESIDUUM_PREDICATE_SPEC.md](../../../RESIDUUM_PREDICATE_SPEC.md).
...
//! Residuum rules (e.g. `Absent != x` is **false**, not true).
...
/// Comparison operators (Residuum Predicate Profile §6).
...
/// Path resolution result (Residuum §5).
...
fn compare(op: CompareOp, left: &Resolve, right: &Resolve) -> bool {
    ...
    match op {
        ...
        Ne => match (left, right) {
            // Absent != anything ⇔ false (Residuum §6.2)
            (Resolve::Absent, _) | (_, Resolve::Absent) => false,
            (Resolve::Present(a), Resolve::Present(b)) => !json_eq(a, b),
        },
