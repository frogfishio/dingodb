...
//!
//! Builtin ids: `sda`, `dql` (official human dialect → ENR1+SDA), `json`,
...
impl BuiltinDialect {
    ...
    pub const fn description(self) -> &'static str {
        match self {
            ...
            Self::Dql => "Official human dialect; lowers to ENR1 Match/enrich/cardinality",
    ...
    pub fn compile(self, source: &str) -> Result<CompiledSda, Error> {
        match self {
            ...
            Self::Dql => dql::compile_dql(source),
            ...
            Self::Graphql => Err(Error::QueryInvalid(
                "dialect 'graphql' is reserved but not implemented; \
                 use pure SDA, dql, json/mongo filter, or sql mimicry (see doc/SDA/DIALECTS.md)"
                    .into(),
            )),
        }
    }
...
pub fn compile_dialect(dialect_id: &str, source: &str) -> Result<CompiledSda, Error> {
    match BuiltinDialect::from_id(dialect_id) {
        ...
        None => Err(Error::QueryInvalid(format!(
            "unknown query dialect {dialect_id:?}; known: sda, dql, json, mongo, sql, graphql \
