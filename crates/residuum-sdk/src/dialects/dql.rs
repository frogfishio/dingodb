...
//! (`residuum-sda`); there is no second evaluator.

use crate::error::Error;
use crate::dialects::CompiledSda;

/// Compile DQL source into pure ENR1 + SDA program text.
pub fn compile_dql(source: &str) -> Result<CompiledSda, Error> {
    let trimmed = source.trim();
    if trimmed.is_empty() {
        return Err(Error::QueryInvalid("dialect 'dql': empty program".into()));
    }
    let program = parse_dql(trimmed)?;
    ...
    sda_core::Program::parse(&sda).map_err(|e| {
        Error::QueryInvalid(format!(
            "dialect 'dql': internal SDA compile failed: {e}; sda={sda}"
        ))
    })?;
...
fn parse_dql(source: &str) -> Result<DqlProgram, Error> {
        ...
        match c {
            ...
            other => {
                return Err(Error::QueryInvalid(format!(
                    "dialect 'dql': unexpected character {other:?}"
                )));
            }
        }
    }
...
impl Parser {
    ...
    fn expect_ident(&mut self) -> Result<String, Error> {
        match self.advance() {
            ...
            other => Err(Error::QueryInvalid(format!(
                "dialect 'dql': expected identifier, got {other:?}"
            ))),
        }
    }
    ...
    fn expect_kw(&mut self, kw: &str) -> Result<(), Error> {
        match self.advance() {
            ...
            other => Err(Error::QueryInvalid(format!(
                "dialect 'dql': expected keyword {kw:?}, got {other:?}"
            ))),
        }
    }

    fn parse_program(&mut self) -> Result<DqlProgram, Error> {
        let mut from = None;
        ...
        while !matches!(self.peek(), Tok::Eof) {
            match self.peek() {
                ...
                Tok::Ident(s) if s.eq_ignore_ascii_case("project") => {
                    if project.is_some() {
                        return Err(Error::QueryInvalid(
                            "dialect 'dql': only one project clause is allowed".into(),
                        ));
                    }
                    self.advance();
                ...
                other => {
                    return Err(Error::QueryInvalid(format!(
                        "dialect 'dql': unexpected token {other:?}; expected from/enrich/project"
                    )));
                }
            }
        }
        ...
        if from.is_none() && enriches.is_empty() {
            return Err(Error::QueryInvalid(
                "dialect 'dql': program needs `from <collection>` or at least one enrich"
                    .into(),
            ));
        }
    ...
    fn parse_enrich(&mut self) -> Result<EnrichStep, Error> {
        ...
        match self.advance() {
            ...
            other => {
                return Err(Error::QueryInvalid(format!(
                    "dialect 'dql': expected '=' in matching clause, got {other:?}"
                )));
            }
        }
        ...
        let cardinality = match card.to_ascii_lowercase().as_str() {
            ...
            other => {
                return Err(Error::QueryInvalid(format!(
                    "dialect 'dql': unknown expect cardinality {other:?}; \
    ...
    fn parse_project_block(&mut self) -> Result<Vec<ProjectField>, Error> {
        match self.advance() {
            ...
            other => {
                return Err(Error::QueryInvalid(format!(
                    "dialect 'dql': expected '{{' after project, got {other:?}"
                )));
            }
        }
        ...
        match self.advance() {
            ...
            other => {
                return Err(Error::QueryInvalid(format!(
                    "dialect 'dql': expected '}}' to close project, got {other:?}"
                )));
            }
        }
        Ok(fields)
...
fn lower_to_sda(program: &DqlProgram) -> Result<String, Error> {
    let root = program
        .from
        .clone()
        .ok_or_else(|| Error::QueryInvalid(
            "dialect 'dql': `from <collection>` is required for v0.1 lowering".into(),
        ))?;
...
fn lower_project_fields(
    ...
    if depth > 8 {
        return Err(Error::QueryInvalid(
            "dialect 'dql': project nesting too deep".into(),
        ));
    }
                    ...
                    if is_last {
                        ...
                        match entry {
                            ...
                            Node::Leaf(_) | Node::Nested(_) => {
                                return Err(Error::QueryInvalid(format!(
                                    "dialect 'dql': project path conflict at {seg}"
                                )));
                            }
                        }
                    }
    ...
    fn enrich_exactly_one_lowers_and_runs() {
        let dql = r#"
            ...
            enrich customer using customers
              ...
              expect exactly_one
        "#;
        let compiled = compile_dql(dql).unwrap();
