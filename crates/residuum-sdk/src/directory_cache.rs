...
//!
//! This module is deliberately free of `residuum-cluster` so remote routing stays
//! available on the MPL embedded/remote SDK path. Wire types use plain integers;
//! in-process cluster code converts [`residuum_cluster`] directories under the
...
/// Dense node index within a cluster (wire form of `residuum_cluster::NodeId`).
...
/// Virtual partition identifier (wire form of `residuum_cluster::PartitionId`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize, Deserialize)]
...
impl DirectorySnapshot {
    /// Build from a live `residuum-cluster` placement directory and optional endpoints.
    ///
    /// Available only when the `cluster` feature is enabled (AGPL `residuum-cluster`).
    #[cfg(feature = "cluster")]
    pub fn from_cluster_directory(
        dir: &residuum_cluster::PartitionDirectory,
...
impl ClientDirectoryCache {
    ...
    /// Build a cache from a `residuum-cluster` placement directory (no network endpoints).
    #[cfg(feature = "cluster")]
    pub fn from_directory(dir: &residuum_cluster::PartitionDirectory) -> Self {
    ...
    /// Replace the entire cache from a `residuum-cluster` directory + endpoints.
    #[cfg(feature = "cluster")]
    pub fn replace(
        &mut self,
        dir: &residuum_cluster::PartitionDirectory,
    ...
    #[cfg(feature = "cluster")]
    pub fn refresh_entry(&mut self, assignment: &residuum_cluster::PartitionAssignment) {
