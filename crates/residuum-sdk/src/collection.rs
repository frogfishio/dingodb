...
use residuum_store::{PayloadResult, Store};
use serde::Serialize;
...
impl<'a> Collection<'a> {
    ...
    ///
    /// Default durability is [`residuum_store::DurabilityMode::Durable`].
    ...
    pub fn get_version(
        ...
        key: &str,
        event_id: &[u8; 16],
    ) -> Result<residuum_store::VersionedPayloadResult, Error> {
        let subject = encode_subject(&self.name, key)?;
        let subject_str = str::from_utf8(&subject).expect("stage 4 subject is UTF-8");
        let store = self
            .local_store()
        ...
        Ok(store.get_payload_version(
            subject_str,
            event_id,
            residuum_store::ReadBudget::default(),
        )?)
    }
    ...
    pub fn find_last_complete(
        &mut self,
        key: &str,
        options: residuum_store::RecoveryReadOptions,
    ) -> Result<residuum_store::HistoricalSearchResult, Error> {
        let subject = encode_subject(&self.name, key)?;
        let subject_str = str::from_utf8(&subject).expect("stage 4 subject is UTF-8");
        let store = self
            .local_store()
        ...
        Ok(store.find_last_complete_version(
            subject_str,
            residuum_store::BeforeEvent::Current,
            options,
        )?)
    }
    ...
    pub fn scan_keys(&mut self) -> Result<Vec<String>, Error> {
        match self.backend {
            Backend::Local(_) => {
                ...
                let page_size = residuum_store::DEFAULT_PAGE_SIZE;
                loop {
    ...
    /// [`residuum_store::Store::scan_live_page`] / `scan_live_logical` for partial maps.
    /// Embedded only.
    ///
    /// **DEF-026:** pages through the store with a bounded page size (default
    /// [`residuum_store::DEFAULT_PAGE_SIZE`]) instead of materializing the full
    ...
    pub fn scan_json_iter_paged(&mut self, page_size: usize) -> Result<JsonScanIter<'_>, Error> {
        ...
        Ok(JsonScanIter {
            ...
            collection: name,
            prefix,
            page_size: page_size.clamp(1, residuum_store::MAX_PAGE_SIZE),
            continuation: None,
        ...
        match self.backend {
            ...
            Backend::Local(_) | Backend::Remote(_) => {
                ...
                Ok(crate::ClusterFindResult {
                    rows,
                    coverage: residuum_cluster::Coverage::default(),
...
pub fn find_on_store(
    ...
    let page_size = options
        .limit
        .unwrap_or(residuum_store::DEFAULT_PAGE_SIZE)
        .clamp(1, residuum_store::MAX_PAGE_SIZE);
    loop {
...
fn scan_json_page_on_store(
    ...
    use residuum_store::LiveScanPageOptions;
...
fn scan_json_partial_page_on_store(
    ...
    use residuum_store::{IncompleteReason, LiveScanPageOptions};
