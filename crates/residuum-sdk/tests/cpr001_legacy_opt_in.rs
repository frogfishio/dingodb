...
use residuum_sdk::{
    ...
    product_may_advertise_qualified_heap, Residuum, DingoDeployment, FLAT_COLLECTION_SURFACE_LABEL,
...
fn legacy_flat_feature_is_opt_in_and_labelled() {
    ...
    assert!(FLAT_COLLECTION_SURFACE_LABEL.contains("not residuum-heap-v1"));
...
fn flat_open_and_deployment_host_both_available() {
    ...
    {
        let mut db = Residuum::open(&flat_path).expect("legacy open");
        let mut users = db.collection("users").expect("flat collection");
        users
            .put("k", &serde_json::json!({"v": 1}))
            .expect("flat put");
    }
    let _ = Residuum::open_compatibility(&flat_path).expect("open_compatibility");

    let deployment = Residuum::open_deployment(&dep_path).expect("open_deployment");
    assert_eq!(deployment.root(), dep_path.as_path());
    let dep2_path = dir.path().join("dep2.dingo");
    let _ = Residuum::create_deployment(&dep2_path).expect("create_deployment");
...
fn raw_store_access_documented_as_non_qualified() {
    ...
    let db = Residuum::open(&path).unwrap();
