...
use residuum_server::{AuthzPolicy, FEATURE_RAFT_RPC_V1, RaftServerState, SDK_RAFT_RPC_PROFILE, ServeOptions, SharedRaftState, TcpRaftTransport, serve_store_with};
...
fn tcp_request_vote_and_append_across_three_peers() {
    ...
    let mut req = residuum_sdk::RpcRequest {
        id: 0,
    ...
    match client1.call_rpc(req) {
        ...
        Err(e) => {
            ...
            assert!(
                e.code() == residuum_sdk::ErrorCode::ConsistencyViolation
...
fn unauthenticated_raft_rpc_rejected_when_token_required() {
    ...
    match client {
        Err(e) => {
            let c = e.code();
            assert!(
                c == residuum_sdk::ErrorCode::AuthenticationFailed
                    || c == residuum_sdk::ErrorCode::Internal
                    || format!("{e}").contains("auth"),
                "expected auth failure, got {e}"
            );
        }
        Ok(mut c) => {
            let req = residuum_sdk::RpcRequest {
                id: 1,
                op: "raft_read_index".into(),
                collection: None,
                key: None,
