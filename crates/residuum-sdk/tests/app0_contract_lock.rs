...
use residuum_sdk::{
    ...
    DQL_APP_CORE_PROFILE, DQL_PLAN_PROFILE, ErrorCode, HeapClient, Parameters, PREDICATE_PROFILE,
    QueryRunOptions, RUST_APP_PROFILE,
};
use residuum_heap::{HeapId, CollectionId};
...
fn app0_error_mapping_codes_are_sdk_codes() {
    ...
    assert!(found_feature, "must map unsupported DQL feature → query_invalid + dql_feature_unavailable");
}
...
fn app0_fixtures_match_op_ids() {
    ...
    let dql = read_json("spec/heap/fixtures/dql_query.accepted.json");
    assert_eq!(dql["op"].as_u64(), Some(118));
    assert_eq!(dql["ok"].as_bool(), Some(true));
    assert!(dql["result"]["exhausted"].as_bool().is_some());
...
fn app0_rust_compile_surface() {
    ...
    assert_eq!(DQL_APP_CORE_PROFILE, "dql-app-core-v1");
    assert_eq!(DQL_PLAN_PROFILE, "dql-plan-v1");
    ...
    let err = col.dql("from orders", &params, opts).unwrap_err();
