...
use residuum_server::{ServeOptions, serve_store_with, validate_bind};
...
fn constant_time_and_redact() {
    assert!(residuum_sdk::constant_time_str_eq("abc", "abc"));
    assert!(!residuum_sdk::constant_time_str_eq("abc", "abd"));
    assert_eq!(residuum_sdk::redact_secret("super-secret"), "***");
    let pki = Pki::new();
