...
use residuum_store::{DurabilityMode, IncompleteReason, PayloadResult, Store};
...
fn key_scan_lists_keys_without_body_reassembly() {
    ...
    let opts = residuum_store::LiveScanPageOptions::new(10);
...
fn document_page_keeps_key_when_body_partial() {
    ...
    let page = store
        .scan_live_documents_page(&residuum_store::LiveScanPageOptions::new(10))
        .unwrap();
...
fn dual_keys_one_chunked_one_inline_partial_page_streams_healthy() {
    ...
    let keys = store
        .scan_live_keys_page(&residuum_store::LiveScanPageOptions::new(2))
        .unwrap();
    assert_eq!(keys.keys.len(), 2);
    assert!(keys.has_more);
    let keys2 = store
        .scan_live_keys_page(
            &residuum_store::LiveScanPageOptions::new(2)
...
fn empty_store_key_scan_is_complete() {
    ...
    let page = store
        .scan_live_keys_page(&residuum_store::LiveScanPageOptions::new(16))
        .unwrap();
...
fn cursor_tamper_rejected_on_key_scan() {
    ...
    tok[last] ^= 0xff;
    let err = store
        .scan_live_keys_page(&residuum_store::LiveScanPageOptions::new(2).continuation(tok))
        .unwrap_err();
    assert!(matches!(err, residuum_store::StoreError::CursorInvalid(_)));
}
