...
fn matrix_records_unqualified_claim_and_mandatory_structure() {
    ...
    assert_eq!(QUALIFIED_PROFILE, "residuum-heap-v1");
    ...
    assert_eq!(doc.format, "residuum-heap-qualification-matrix-v1");
    assert_eq!(doc.package, "HP-010");
    assert_eq!(doc.profile, "residuum-heap-v1");
...
fn mint_cap(slot: Arc<HeapSlot>) -> residuum_heap::HeapCap {
    let snap = slot.load();
...
fn security_revision_bump_terminates_without_lifecycle_helper() {
    ...
    let snap = HeapSecuritySnapshot {
        ...
        heap_id: heap,
        authority_epoch: residuum_heap::AuthorityEpoch::new(1).unwrap(),
...
fn h6_pure_proof_bundle_connected() {
    ...
    let pure = fs::read_to_string(
        workspace_root().join("crates/residuum-heap/src/pure_proofs.rs"),
    )
    .unwrap();
    assert!(
