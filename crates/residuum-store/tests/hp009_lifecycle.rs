...
fn retention_scheduler_blocks_purge_until_window_elapses() {
    ...
    let mut sched = residuum_store::RetentionScheduler::new();
...
fn aws_kms_config_surface() {
    ...
    assert_eq!(cfg.backend, residuum_store::HsmBackendKind::AwsKms);
    assert_eq!(cfg.key_label.as_deref(), Some("alias/residuum-heap"));
    ...
    #[cfg(feature = "aws-kms")]
    {
        ...
        let err = residuum_store::AwsKmsDataKeyProvider::from_config(&cfg);
...
fn data_key_provider_hsm_scaffold_and_in_process() {
    ...
    let aws = HsmDataKeyProvider::from_config(HsmDataKeyConfig::unconfigured(
        residuum_store::HsmBackendKind::AwsKms,
    ));
