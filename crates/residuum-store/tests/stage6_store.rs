...
use residuum_store::{DurabilityMode, EventKind, PayloadResult, Store, DEFAULT_CHUNK_SIZE};
use std::fs;
...
fn chunked_put_get_and_partial_missing_middle() {
    use residuum_format::{decode_chunk_body, scan_forward, FrameKind, SafetyLimits};
    use residuum_store::{decode_chunk_manifest, reassemble_with_manifest, resolve_piece};
    ...
    let raw = store
        ...
        .map(|(_, b)| b.to_vec())
        .unwrap();
    assert!(residuum_store::is_chunk_manifest(&raw));
...
fn secondary_index_create_lookup_delete() {
    use residuum_store::{IndexState, SecondaryIndex};
