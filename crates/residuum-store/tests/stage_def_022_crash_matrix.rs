...
fn crash_child_bin() -> PathBuf {
    ...
    if let Ok(p) = std::env::var("CARGO_BIN_EXE_residuum_store_crash_child") {
    ...
    exe.push("residuum-store-crash-child");
    if exe.is_file() {
        return exe;
    }
    ...
    alt.push("residuum-store-crash-child");
    alt
}
...
fn run_crash_child(
    ...
    assert!(
        bin.is_file(),
        "crash child binary missing at {} (build residuum-store-crash-child first)",
        bin.display()
    );
...
fn run_cell(
    ...
    failpoint: &str,
    fault: Option<&str>,
    expected: &residuum_store::ExpectedReopen,
) {
    clear_failpoints();
...
fn assert_prior_ok(store: &Store, expected: &residuum_store::ExpectedReopen) {
...
fn run_store_create(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    arm_error(failpoint);
...
fn run_put_process_kill(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
...
fn run_put_buffered(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).expect("open");
    arm_error(failpoint);
...
fn run_delete_durable(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    {
...
fn run_chunked_put(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).unwrap();
...
fn run_seal(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    {
        let mut store = Store::open(path).unwrap();
        store
...
fn run_dedup(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).unwrap();
    let receipt = store
        ...
        .unwrap();
    let op_id = [7u8; 16];
    let content = residuum_store::content_identity("put", "", "dedup-k", b"dedup-v");
    arm_error(failpoint);
...
fn run_catalog(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).unwrap();
    store
...
fn run_checkpoint(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let store = Store::open(path).unwrap();
    arm_error(failpoint);
...
fn run_tier_move(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).unwrap();
    store
    ...
    let _ = store.transfer_segment_to_tier(
        ids[0],
        residuum_store::TierClass::Warm,
        residuum_store::TierMoveMode::Copy,
    );
    drop(store);
    clear_failpoints();
    let store = Store::open(path).unwrap();
...
fn run_compact(path: &Path, failpoint: &str, expected: &residuum_store::ExpectedReopen) {
    seed_prior(path);
    let mut store = Store::open(path).unwrap();
    store
