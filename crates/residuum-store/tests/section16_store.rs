...
//! corpus in `residuum-format` (FORMAT_SPEC §13). For every case: surviving
//! islands remain readable, corrupt bytes are not trusted as live state, and
//! derived catalogs/indexes are not required.

use residuum_format::{FRAME_PREFIX_LEN, FRAME_SUFFIX_LEN, START_MAGIC};
...
fn store_descriptor_written_and_readable() {
    ...
    let report = residuum_format::scan_forward(&bytes, residuum_format::SafetyLimits::default());
    let kinds: Vec<_> = report
        .verified_frames()
        .filter_map(|(_, f)| f.header.known_kind())
        .collect();
    assert!(kinds.contains(&residuum_format::FrameKind::StoreDescriptor));
