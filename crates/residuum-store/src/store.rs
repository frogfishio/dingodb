...
const META_VERSION: &str = "residuum-store-9\n";
...
impl Store {
    ...
    pub fn open_with_options(
        ...
        if !meta.starts_with("residuum-store-") {
        ...
        if let Some(w) = self.find_active_by_segment(&lv.segment_id) {
            ...
            if off < bytes.len() {
                if let Ok((_h, _e, body, _hash, _len)) =
                    residuum_format::verify_frame_at(&bytes[off..], self.limits)
                {
    ...
    /// (`residuum-examine`) to project [`residuum_format`] salvage regions into
    ...
    fn write_chunked_put(
        ...
        let mut new_chunk_locs: Vec<([u8; 16], u64, &residuum_format::ChunkPiece)> =
            Vec::with_capacity(pieces.len());
        {
                ...
                .zip(chunk_event_ids.iter().zip(chunk_envelopes.iter()))
            {
                ...
                let header = FrameHeader {
                    wire_major: residuum_format::WIRE_MAJOR,
                    wire_minor: residuum_format::WIRE_MINOR,
...
fn recover_active_bytes(
    ...
    for region in &report.regions {
        match region {
            residuum_format::ScanRegion::VerifiedFrame { range, frame } => {
                ...
                if frame.header.known_kind() == Some(FrameKind::SegmentDescriptor) {
                    if let Some((ids, _, _)) = residuum_format::decode_descriptor_body(&frame.body) {
            ...
            residuum_format::ScanRegion::Hole { .. } => {
...
fn rebuild_active_from_bytes(
    ...
    if !kept.is_empty() {
        ...
        for (_r, frame) in report.verified_frames() {
            if frame.header.known_kind() == Some(FrameKind::SegmentDescriptor) {
                if let Some((_, ns, _)) = residuum_format::decode_descriptor_body(&frame.body) {
                    created_ns = ns;
                }
            }
        }
