...
fn seal_pending_bytes(
    ...
    for region in &report.regions {
        match region {
            ...
            residuum_format::ScanRegion::Hole { .. } => break,
        }
    }
    ...
    .map_err(|e| StoreError::CorruptMeta(match e {
        residuum_format::SegmentError::MissingDescriptor => "pending missing descriptor",
        residuum_format::SegmentError::AlreadySealed => "pending already sealed",
...
fn write_chimera_from_segment_puts(
    ...
    use residuum_format::scan_forward;
...
mod tests {
    use super::*;
    use residuum_format::{ActiveSegment, FrameKind, SegmentId};
    ...
    fn seal_pending_preserves_prefix_bytes() {
        ...
        let (_h, _e, body, _, _) =
            residuum_format::verify_frame_at(&sealed[off as usize..], SafetyLimits::default())
                .unwrap();
