...
pub fn transfer_segment(
    ...
    if src == dest {
        ...
        return Ok(MigrationEvidence {
            ...
            dest_hash: hash,
            size,
            tool_version: "residuum-store-9".into(),
            migrated_ns: now_ns(),
        });
    }
...
pub fn classify_segment_bytes(bytes: &[u8]) -> FormatClassification {
    use residuum_format::{scan_forward, FrameVerifyError, SafetyLimits, WIRE_MAJOR};
    ...
    for region in &report.regions {
        match region {
            residuum_format::ScanRegion::VerifiedFrame { frame, .. } => {
            ...
            residuum_format::ScanRegion::Hole { reason, .. } => {
                if let residuum_format::HoleReason::CorruptCandidate {
