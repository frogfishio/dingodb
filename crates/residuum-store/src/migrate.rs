...
/// `residuum-client::PROTOCOL_MAJOR`; kept here so the store crate does not
/// depend on the client).
pub const PROTOCOL_MAJOR_DECLARED: u16 = 1;

/// Declared network protocol minor (mirrors `residuum-client::PROTOCOL_MINOR`).
pub const PROTOCOL_MINOR_DECLARED: u16 = 0;

/// Declared RPC profile tag (mirrors `residuum-client::PROTOCOL_PROFILE`).
pub const PROTOCOL_PROFILE_DECLARED: &str = "dingo-rpc-v1";

/// Draft RPC interoperability label (mirrors `residuum-client::RPC_WIRE_LABEL`).
    ...
    wire_compat_matrix()
        ...
        .map(|e| WireMatrixRow {
            ...
            status: match e.status {
                residuum_format::WireSupportStatus::Current => "current".into(),
                residuum_format::WireSupportStatus::ReadOnly => "read_only".into(),
                residuum_format::WireSupportStatus::Deprecated => "deprecated".into(),
                residuum_format::WireSupportStatus::Unsupported => "unsupported".into(),
            },
        })
        .collect()
}
