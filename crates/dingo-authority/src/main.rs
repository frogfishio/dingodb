//! `dingo-authority` — local-only heap authority ceremony CLI (AGPL).
...
fn main() -> ExitCode {
    ...
    match cli.command {
        None => {
            eprintln!("usage: dingo-authority <genesis|issue|reload|--license>");
            ExitCode::FAILURE
        }
