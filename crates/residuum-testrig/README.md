...
```sh
cargo build -p residuum-testrig --release
# binary: target/release/residuum-testrig
```

## Quick smoke (small target)

```sh
cargo run -p residuum-testrig -- run \
  --work /tmp/residuum-testrig-smoke \
  --target-bytes 8M \
  --payload-size 2048 \
  --chaos-hits 8 \
  --seed 1
...
```sh
cargo run -p residuum-testrig --release -- run \
  --work /var/tmp/residuum-testrig-1g \
  --target-bytes 1G \
  --payload-size 4096 \
  --durability buffered \
  --chaos-hits 32 \
...
cargo run -p residuum-testrig --release -- run \
  --work /var/tmp/residuum-testrig-10g \
  --target-bytes 10G \
  --payload-size 8192 \
  --chaos-hits 64
...
```sh
# 1) fill
residuum-testrig pump --store /tmp/s --target-bytes 1G

# 2) baseline health
residuum-testrig monitor --store /tmp/s --phase baseline

# 3) close all writers, then punch holes
residuum-testrig chaos --store /tmp/s --hits 16 --seed 42

# 4) post-damage
residuum-testrig monitor --store /tmp/s --phase post-chaos --inspect-only
```
...
| Hook | Role |
|------|------|
| `residuum-store` `stage6_bench_skeleton` | Tiny hot-path CI skeleton |
| `residuum-store` `stage9_archive_bench` | Archive-path class skeleton |
| `scripts/demos/02_punch_a_hole.sh` | Narrative single-hole demo |
| **residuum-testrig** | Scale + multi-hit chaos + metrics ladder |
