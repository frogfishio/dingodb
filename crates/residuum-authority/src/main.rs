//! `residuum-authority` — local-only heap authority ceremony CLI (AGPL).
...
fn main() -> ExitCode {
    ...
    match cli.command {
        None => {
            eprintln!("usage: residuum-authority <genesis|issue|reload|--license>");
            ExitCode::FAILURE
        }
