...
use residuum_heap::{DeploymentId, HeapId, Rights};
use residuum_store::{try_load_heap_catalog, HeapMetaLayout};
use std::fs;
...
fn staged_only_before_authority_commit_is_invisible() {
    ...
    let (deployment, heap, creation) = ids();
    let staged = residuum_store::stage_heap_genesis(&layout, deployment, heap, creation, "ghost")
        .unwrap();
    assert!(try_load_heap_catalog(&layout).unwrap().is_none());
    // Authority never committed — published discovery stays empty.
    assert!(residuum_store::load_staged_genesis(&layout, &staged.staging_id)
        .unwrap()
        .is_some());
}
...
fn equal_sequence_unequal_payloads_are_fork() {
    ...
    residuum_authority::AuthorityHead::decode_payload(&payload_a).unwrap();

    let mut head_b = head.clone();
    head_b.current_descriptor_hash = [0xABu8; 32];
    let payload_b = head_b.encode_payload().unwrap();
    residuum_authority::AuthorityHead::decode_payload(&payload_b).unwrap();
    ...
    fs::write(
        ...
        residuum_authority::slot_encode_for_test(&payload_a),
    )
    .unwrap();
    fs::write(
        paths.heap_dir().join("head.b.cbor"),
        residuum_authority::slot_encode_for_test(&payload_b),
    )
    .unwrap();
...
#[test]
fn residuum_server_does_not_depend_on_residuum_authority() {
    let toml = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../residuum-server/Cargo.toml"),
    )
    .unwrap();
    assert!(
        !toml.contains("residuum-authority"),
        "residuum-server must not depend on residuum-authority"
    );
    assert!(
        !toml.contains("authority-provisioning"),
        "residuum-server must not enable authority-provisioning"
    );
}
