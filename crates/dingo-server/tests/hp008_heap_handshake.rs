...
use dingo_client::{b64u_encode, HeapAuth, HeapReject, FEATURE_HEAP_KEY_V1};
...
fn hot_path_modules_do_not_reference_authority_store() {
    ...
    for name in [
        ...
        assert!(
            !src.contains("dingo_authority") && !src.contains("MasterAuthorityStore"),
...
fn qualified_session_welcome_and_reject_are_indistinguishable_on_wire() {
    ...
    dingo_client::write_frame(&mut server_in, &auth_body).unwrap();
    ...
    let mut server_in = Vec::new();
    write_json_frame(&mut server_in, &hello).unwrap();
    dingo_client::write_frame(&mut server_in, &bad_auth).unwrap();
    let mut reader = Cursor::new(server_in);
    let mut writer = Vec::new();
