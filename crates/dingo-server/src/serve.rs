...
//!
//! Extracted from the former monolithic `dingo-sdk` remote module.
...
fn server_handshake_buffered(reader: &mut BufReader<IoStream>) -> Result<NegotiatedSession, Error> {
    ...
    let hello = match dingo_sdk::parse_handshake(&payload) {
        Ok(h) => h,
        Err(e) => {
            let _ = write_json_frame(
                reader.get_mut(),
                &dingo_sdk::Handshake::reject("protocol_violation", e.to_string()),
            );
            return Err(e);
        }
    };
    if hello.msg != dingo_sdk::HandshakeMsg::Hello {
        let msg = format!("expected hello, got {:?}", hello.msg);
        let _ = write_json_frame(
            reader.get_mut(),
            &dingo_sdk::Handshake::reject("protocol_violation", &msg),
        );
        return Err(Error::ProtocolViolation(msg));
    }
    if hello.v != dingo_sdk::PROTOCOL_MAJOR
        && hello.protocol_major.unwrap_or(hello.v) != dingo_sdk::PROTOCOL_MAJOR
    {
        let msg = format!(
            "unsupported protocol major {} (server speaks {})",
            hello.v,
            dingo_sdk::PROTOCOL_MAJOR
        );
        let _ = write_json_frame(
            reader.get_mut(),
            &dingo_sdk::Handshake::reject("protocol_version_unsupported", &msg),
        );
        return Err(Error::ProtocolViolation(msg));
    }
    let features = match dingo_sdk::negotiate_features(hello.features.as_deref().unwrap_or(&[])) {
        Ok(f) => f,
        Err(e) => {
            let _ = write_json_frame(
                reader.get_mut(),
    ...
    let max_frame = dingo_sdk::negotiate_max_frame(hello.max_frame);
    write_json_frame(
        reader.get_mut(),
        &dingo_sdk::Handshake::welcome(max_frame, features.clone()),
    )?;
    Ok(NegotiatedSession {
        max_frame,
        features,
        protocol_major: dingo_sdk::PROTOCOL_MAJOR,
        protocol_minor: dingo_sdk::PROTOCOL_MINOR,
    })
}
...
fn dispatch(
    ...
    match req.op.as_str() {
        ...
        "put" => {
            ...
            let limits = dingo_sdk::host_limits();
            dingo_sdk::check_json_depth(json, &limits)?;
            let mode =
            ...
            dingo_sdk::check_payload_len(body.len(), &limits)?;
        ...
        "put_bytes" => {
            ...
            let limits = dingo_sdk::host_limits();
            dingo_sdk::check_payload_len(bytes.len().saturating_add(1), &limits)?;
            let mode =
                parse_durability(req.durability.as_deref()).unwrap_or(DurabilityMode::Durable);
            let subject = encode_subject(coll, key)?;
            let subject_str = str::from_utf8(&subject).expect("stage 4 subject is UTF-8");
