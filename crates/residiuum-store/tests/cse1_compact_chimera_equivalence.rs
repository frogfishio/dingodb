//! CSE-1 — Compact Chimera salvage equivalence campaign.
//!
//! Runs the frozen CSE-0 failure set `F` against **default Compact** seal
//! layouts and compares recoverable sets to the Materialized baseline.
//!
...
//!
//! Honest scope: measure + compare. Does **not** accept Compact as product
//! default. If the inequality fails, CSE-2 is required.
...
/// Seed store with default Compact Chimera (seal path — no Materialized overwrite).
fn seed_compact_fixture() -> Fixture {
    ...
    // Confirm seal wrote Compact (SegmentFrame) layout — not Materialized.
    let store = Store::open(&root).unwrap();
    let loaded = store
        .load_chimera_layout(segment_id)
        .unwrap()
        .expect("compact layout present after seal");
    let counts = loaded.count_by_kind();
    assert!(
        counts.segment_frame >= 3,
        "CSE-1 requires Compact SegmentFrame seal layout (got {counts:?})"
    );
    assert_eq!(counts.inline + counts.point_container + counts.large_value_log, 0);
    // In-layout get cannot resolve SegmentFrame without store pread.
    assert!(
...
fn cse1_compact_equivalence_campaign() {
    ...
    let summary = format!(
        "{{\"package\":\"CSE-1\",\"equivalence_holds\":{},\"requires_cse2\":{},\"gaps\":[{}],\"compact\":[{}],\"materialized_rhs\":\"CSE-0 baseline.json\"}}",
        equivalence_holds,
        !equivalence_holds,
        gaps.iter()
    ...
    assert!(
        !equivalence_holds,
        "unexpected: Compact held Materialized recoverability — re-check CSE-2 need; gaps={gaps:?}"
    );
    assert!(
