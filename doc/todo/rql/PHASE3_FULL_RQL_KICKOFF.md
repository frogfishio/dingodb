...
Status: **labor 2026-08-05** · kickoff `89a80e77` + T3.2–**T3.5**  
...
| Nested `within` / enrich after within / multi top-level within | **residual** |
...
Multiple root enrich steps and multiple enrich steps inside one within are ordered attach.
...
Evidence: `doc/todo/rql/evidence/phase3_multi_enrich.log`

## Non-claims

- Not full RQL-v1 product accept.
- Not APB-7 package accept.
- Nested `within`, enrich-after-within, and multi top-level within still residual.
- Façade is HeapClient-local scan attach — not remote op-118 enrich wire.

## Next slices

1. Nested `within` depth (and/or enrich after within).
2. Only then `at rank` / access policies (DDA-dependent).
