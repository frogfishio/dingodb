...
**QVM1** durable encoding + `VmPool` landed. Honest residual: `RQB1` is still the
public AST carrier; two dispatch loops remain; sql/json/mongo dialects still
compile to SDA.
...
- QVM1 ≠ one `run_vm` / Decision 0 closed
- Prior VM1 / P1c claims ≠ accepted
- **RQL-C1 must not be accepted**
- NEXT labor = **VM1R** then dialect→QVM — **not** principal C1
