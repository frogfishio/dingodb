...
Status: **archived snapshot as of 2026-07-27**
Audience: operators of the engineering program (humans + agents)
Companion: [DELIVERY_PLAN.md](./DELIVERY_PLAN.md), [DEFECTS.md](../incidents/DEFECTS.md),
[CAPABILITY_MATRIX.md](../../wip/status/CAPABILITY_MATRIX.md),
...
From [DEFECTS.md](../incidents/DEFECTS.md) and §16 release gates — still open or only
partially cut:
...
| **ENR1 (in `residiuum-sda`)** | Match-bag **kernel + surface** shipped (`sda-enr1-v0.1`: `Match`/`enrich`/`one?`/`one!`/`merge`/`mergeLeft`/`mergeRight`/`+`/`asBag`/keyed sugar/`source` decls/`t_enr_invalid_key`). Multi-gen expand still blocked on SDA multi-gen. **RQL** v0.1 official dialect compiles to the same kernel (`compile_dialect("rql", …)`); proof tests equate RQL ≡ pure ENR. Foreign dialects absorb comfort. **Do not open ENR2 yet.** |
...
| **DEF-063** | **Threat-model first cut** — `doc/wip/security/THREAT_MODEL.md`; independent audit + disclosure policy remain. |
...
From [CAPABILITY_MATRIX.md](../../wip/status/CAPABILITY_MATRIX.md) and DEFECTS §2:
...
(`doc/reference/operations/BENCHMARK_DISCLOSURE.md`) put the **steady-state write index path** past
...
`residiuum-testrig run --target-bytes 10G --payload-size 8192 --seed 2` → **PASS**.
...
`/var/tmp/residiuum-testrig-10g/testrig-summary.v1.json`. Diagnostic only.

**Sharded re-measure (2026-07-27, Axis B harness):**
`residiuum-testrig run --target-bytes 10G --payload-size 8192 --seed 2
...
`/var/tmp/residiuum-testrig-10g-shards/testrig-summary.v1.json`. Diagnostic only.
...
Design authority: [`PARALLEL_INGEST.md`](../../reference/operations/PARALLEL_INGEST.md). Do not spend the
...
3. **Capacity ≠ efficiency.** Multi-store proves media and cores can be used;
   product multi-tenant / multi-node is still residiuum-cluster.
...
| **S3** | **Productize capacity on the cluster path** | Independent store/partition leaders (residiuum-cluster), honest RF/ack modes, network repair/query only as needed for multi-process proof | Treat testrig `--stores N` as multi-tenant product sharding |
...
Companion detail: [`PARALLEL_INGEST.md`](../../reference/operations/PARALLEL_INGEST.md) §10,
...
2. **1 GiB numbers are diagnostic only** (see
   [BENCHMARK_DISCLOSURE.md](../../reference/operations/BENCHMARK_DISCLOSURE.md)). Single machine, buffered
...
5. **Repo integrity.** Hydra + `residiuum-testrig` sources must live in git with the
...
Today in `Store::put_many_parallel` (`residiuum-store` `store.rs`):
...
Companions: [`PARALLEL_INGEST.md`](../../reference/operations/PARALLEL_INGEST.md) §10, [`BENCHMARK_DISCLOSURE.md`](../../reference/operations/BENCHMARK_DISCLOSURE.md), DEF-023 / DEF-095 / DEF-096 / DEF-093 in [`DEFECTS.md`](../incidents/DEFECTS.md).

**Canonical write-up of all strategies (S1–S6, max-performance residuals, SDA
A1–A5):** [`PERFORMANCE_STRATEGIES.md`](../../reference/operations/PERFORMANCE_STRATEGIES.md).
...
| Hook | Role |
|------|------|
| `residiuum-sda` `Program::parse` + `run_json` / `eval` | Compile-once host API |
| `residiuum-sda` example `sda_latency_breakdown` | Phase attribution (diagnostic) |
| `residiuum-sda` `sda_bench_skeleton` | CI absurdity bounds |
| SDK `Filter::compile_sda` / `matches_compiled_sda` | Multi-doc filter parity without re-parse |

Strategies: [`PERFORMANCE_STRATEGIES.md`](../../reference/operations/PERFORMANCE_STRATEGIES.md) § SDA.
