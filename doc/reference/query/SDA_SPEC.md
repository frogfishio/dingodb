...
[doc/SDA/DIALECTS.md](../../SDA/DIALECTS.md).
...
- If any key appears more than once ->
  `Fail(t_sda_duplicate_key, "duplicate key")`
- Otherwise -> `Ok(map)`

### 7.2 Ordered conflict policies are not BagKV operations

`BagKV` is unordered and its equality is extensional with multiplicity.
Consequently, “first wins” and “last wins” have no lawful meaning for a
`BagKV`: two equal bags may have different physical or parse encounter orders,
and a function that observed those orders would produce different results for
equal inputs.

The core therefore does not define `normalizeFirst` or `normalizeLast`.

A host that needs encounter-order conflict resolution MUST preserve the input
as an ordered `Seq[Bind]` and expose a separately specified operation over that
ordered carrier. Such an operation is not a `BagKV` normalization and MUST NOT
be presented as core SDA behavior.
...
- BagKV does not provide an encounter order.

SDA forbids silent conversion and hidden ordering.
...
- The function passed to `mapOpt` may return any comparable SDA data value.
- The function passed to `bindOpt` MUST return `Opt`; another return kind
  produces `Fail(t_sda_wrong_shape, "wrong shape")`.
- The function passed to `mapRes` may return any comparable SDA data value.
- The function passed to `bindRes` MUST return `Res`; another return kind
  produces `Fail(t_sda_wrong_shape, "wrong shape")`.
- A function argument that is not callable produces
  `Fail(t_sda_not_callable, "not callable")`.
...
let ctRes =
  bindRes(
    normalizeUnique(headers),
    m => m<"content-type">!
  );
```

`ctRes` remains an explicit `Res`: either `Ok(value)` or `Fail(code,msg)`.
SDA does not implicitly unwrap results.
...
To continue processing the converted bag, compose through `bindRes` with a
lambda that itself returns a `Res`.
...
- Normalization:
  ...
  - Reordering a `BagKV` does not change the result of `normalizeUnique`.
  - The core exposes no first-wins or last-wins operation over unordered `BagKV`.
- Equality:
