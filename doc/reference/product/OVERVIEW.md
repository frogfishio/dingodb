# Residiuum Architecture Specification
...
Residiuum is a damage-tolerant, high-performance, long-retention database for
...
Residiuum stores structured records, events, logs, documents, binary objects,
...
**Store**
A logical Residiuum namespace containing items, segments, chunks, derived
...
**Payload**
Structured data or opaque bytes preserved by Residiuum.
...
**Uncertain**
Physical data survives, but Residiuum cannot prove that the derived logical
...
Residiuum MUST recover every independently verifiable frame or segment that
...
Residiuum MUST report discovered discontinuities as holes. It MUST NOT silently
...
event required to reconstruct current state may be missing. Residiuum MUST return
the surviving event. A state projection that depends on the missing event MUST
be marked incomplete or uncertain.

Residiuum MUST NOT discard verified data merely because its full semantic
...
Residiuum MUST be able to preserve opaque bytes without understanding their
...
MUST distinguish stored payloads from references to content outside Residiuum.

### 5.4 Events and state

Residiuum records immutable storage events. Core event kinds are:
...
- identify it as a Residiuum segment;
...
If some chunks are missing, Residiuum MUST return the surviving chunks and an
...
A Residiuum store is logically a segment fabric. Segments may reside:
...
SDA is Residiuum's deterministic examination and transformation algebra.

Residiuum MUST expose an SDA value for:
...
The standalone Residiuum SDA profile exposes examination units with fixed fields
...
defined by the
[Residiuum SDA examination profile](../query/SDA_PROFILE.md).
...
Residiuum defines three distinct performance classes:
...
A Residiuum cluster federates independently recoverable partitions and segments.
...
[CLUSTER_SPEC.md](../../todo/cluster/CLUSTER_SPEC.md).
...
Residiuum exposes a collection-oriented API for JSON and bytes with familiar
