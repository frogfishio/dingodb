...
| Parallel ingest / multi-core write | **Axis A + B + C harness shipped** | Axis A: O(1) rotate + background seal (`seal_pipeline`). Axis B: `create_with_shards(N)`, subject-hash routing, `put_many` parallel append; N=1 keeps legacy `active/active.dingo`. Axis C harness: testrig `--stores N` multi-process child pumps. Testrig: `--writer-shards N` / `--stores N` + disclosure (`concurrency` / `writer_model` / `store_count` / peak RSS+CPU%). Product multi-node = dingo-cluster — `doc/PARALLEL_INGEST.md` / DEF-096 |
