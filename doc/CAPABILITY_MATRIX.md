...
| Steady-state vs lifecycle attribution | shipped | Measured: ordinary put is data+dual-index µs-class; remaining spikes were synchronous `persist_index_cache` / `seal_active` (see `doc/BENCHMARK_DISCLOSURE.md`); auto-seal path now offloads finalize (DEF-096 Axis A) |
