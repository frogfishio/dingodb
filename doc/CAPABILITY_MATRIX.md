...
| Parallel ingest / multi-core write | **Axis A + B shipped** | Axis A: O(1) rotate + background seal (`seal_pipeline`). Axis B: `create_with_shards(N)`, subject-hash routing, `put_many` parallel append; N=1 keeps legacy `active/active.dingo`. Axis C = multi-process — `doc/PARALLEL_INGEST.md` / DEF-096 |
