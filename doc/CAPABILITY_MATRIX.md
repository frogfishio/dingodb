...
| Parallel ingest / multi-core write | **Axis A shipped** | Auto-seal: O(1) rotate to `active/pending/` + background BLAKE3/Hydra/Chimera (`seal_pipeline`). Explicit `seal_active` still sync. Axis B sharded writers open — `doc/PARALLEL_INGEST.md` / DEF-096 |
