...
| Component | Path | Role |
|-----------|------|------|
| Isolation kernel | `crates/residiuum-heap` | decide, caps, constraints, confinement |
| Ownership / admit | `crates/residiuum-format` | SubjectV2, frame admit |
| Store façades / lifecycle | `crates/residiuum-store/src/heap/` | catalogs, purge, restore, migration |
| Local ceremony | `crates/residiuum-authority` | issue / cycle (not linked by server) |
| Qualified network | `crates/residiuum-server` heap_* + client handshake | HeapKey session |
...
| Complete-path review | `doc/wip/heap/HEAP_COMPLETE_PATH_REVIEW.md` | Path inventory |
...
| CPR-004 | Kani + Verus pure-kernel connected in CI (`kani-heap`, `verus-heap`) | Treat formal pure path as connected; still not a substitute for this review |
