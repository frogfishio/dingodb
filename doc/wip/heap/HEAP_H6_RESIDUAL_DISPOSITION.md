...
[HEAP_NEXT_TASKS.md](../../done/programs/HEAP_NEXT_TASKS.md)
...
2. **Engage reviewer** with the brief §5 attack questions; fix time-box and
   TCB boundary (single-node `residiuum-heap-v1`, not cluster).
...
| Flag | Today | Meaning |
|------|-------|---------|
| `KANI_HARNESSES_CONNECTED` | **true** | `residiuum_heap` pure_proofs; CI `kani-heap` |
...
| Verify pure_kernel | `./scripts/check_verus_heap.sh` (or `RESIDIUUM_REQUIRE_VERUS=1`) |
...
Rust lemmas in `residiuum_heap::pure_proofs` remain the product-code stand-ins.
