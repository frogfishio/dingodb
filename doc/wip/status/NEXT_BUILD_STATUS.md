...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Prior accepted: shared workload exec; per-cell reports; bundle. **This slice:** duration-controlled pairs use **sustained throughput** probe cost (not wall-time); every overhead leg must meet floors+validity; matrix dims as behaviour — `create_with_shards`, concurrent preparers, outstanding-bounded `put_many` batches. Evidence: store-driver lib green. **No 120s qual.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | tput cost + real concurrency/batch |
