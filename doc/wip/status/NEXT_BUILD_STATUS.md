...
Updated: 2026-08-03 (**P0 DEF-SCAN-001** T7–T12 accepted labor, package not accepted. **AWO T11 evidence freeze principal `done`.** **AWO-Q series:** Feature `d0ae3c06` — Q0 **done**; **AWO-Q1 package principal `done`**; **AWO-Q2 labor `in_review`** (`0a043642`, evidence `AWO_Q2_DECISION_QUALITY.md` — decision cells + collector detach hang fix). Q3–Q4 still `backlog`. Explicit Q2 non-claims: package accept, default-on, thr floors, PQH diagnostic, sparse product bound, collector `select_plan` wiring. Plan `AWO_QUALIFICATION_SERIES.md`. T11 smoke = mechanism floor only. **Not** AWO product package accept / floors / default-on. Scratch exFAT residual remains. Other AWO measure cards still `in_review` as applicable; query spine / APB **in_review**)
