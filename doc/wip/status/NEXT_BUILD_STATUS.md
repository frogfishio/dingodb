...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Prior accepted: aggregates≠plan; hashed preflight/env; multiproc; store hooks. **This slice:** order-balanced multi-pair probe-off/on on **actual cell workload** (no 64-op qual cap); `OBSERVER_OVERHEAD_BUDGET=0.02` enforced (`within_budget` + product_claim withheld when exceeded); report on `CampaignResult.observer_overhead_report` + hashed `observer_overhead.json`; measured fraction → `RunEvidence.observer_overhead_fraction` / attribution. Evidence: store-driver lib tests green. **No 120s qual.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | observer-overhead qualification |
