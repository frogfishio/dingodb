...
| PQH-11 | active | 2026-07-31 | PQH-10 | multi-rep real_store campaign path (≥5×2), run IDs + primary registered bottleneck in bundle, CLI `--driver real_store` (+ optional `--controlled`); product_claim_eligible only controlled+real; no optimisations; tests default 166 + store-driver multi-rep; **board `in_review`** | principal accept; true controlled-runner host declare; optimisations blocked until principal greenlights | real-store campaign evidence |
