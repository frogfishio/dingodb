...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Accepted: multiproc; encoded_frame_len; no synthesized attribution. **This slice:** store-native `BoundaryProbe` instruments actual write/sync/rotate/publish/lifecycle; perf **consumes** store events (not receipt reconstruction); fail-closed qual preflight gate (`require_qualification_preflight`). Evidence: store 156; perf lib 175; store-driver 179. **No 120s qual run yet.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | store boundary SoT + preflight |
