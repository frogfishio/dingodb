...
Updated: 2026-08-01 (APB-2/APP-3 **active** façade create/upsert/list_keys + replace/delete_with if_version; HAR-1 active; APB-1 G1–G6; APP-5/4 accept)
...
| APB-2 | active | 2026-08-01 | APB-1 | façade **create** / **upsert** / **list_keys** + **replace**/`if_version` + **delete_with** (`if_version`, `if_present`); OCC token = establishing **event_id** (not `WriteReceipt.version` item lineage); `Error::VersionConflict`/`NotFound`; `cargo test -p residiuum-sdk --test apb2_facade_mutations` **2/2** | store Key Atomic CAS, receipt.version=event_id alignment, generated keys (`add`), durability options remote, concurrent/crash matrix; **no package accept** | safe single-key mutation |
...
| APP-3 | active | 2026-08-01 | APP-2, HAR-4 | façade put/get/delete + history + indexes (APB-1) + create/upsert/list_keys + replace/delete_with (APB-2); dual pack residual | APP-2 scoreboard lag; store CAS; receipt.version alignment; remote durability; crash matrices; **no package accept** | typed data/history/index |
