...
Updated: 2026-08-01 (APB-1 **active** G1+G1b+G3+G4 IndexManager+history; APP-5 **accept**; APP-4/APB-0 accept)
...
| APB-1 | active | 2026-08-01 | APB-0, HAR-1 | inventory + **G1+G1b+G3+G4:** sealed backends; `From<Heap|RemoteHeap>`; create/open/list + put/get/delete + history + **IndexManager** both backends; embedded **2/2**; remote façade **1/1** (hp007); [APB1_CLIENT_GAP_INVENTORY.md](../../todo/application-baseline/APB1_CLIENT_GAP_INVENTORY.md) v1.4 | dual shared suite (G6) open; HAR-1 scoreboard stale; **no package accept** | backend-neutral client |
...
3. **Query spine** (principal §0.8): **APP-4/APP-5 = accept**; **APB-1 active** (G1+G1b+G3+G4; dual suite residual);
...
| 5 | **APP-4 → APP-5 → APB-7** | APP-4/5 **accept**; **APB-1 active** (G1–G4 client bind); still need dual suite + APB-6/APP-6 for APB-7 path |
