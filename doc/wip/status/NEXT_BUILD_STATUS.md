...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Qualification accept rejected** (smoke ≠ controlled campaign). Correction: `RunClass` smoke/diagnostic/qualification/soak (SPEC §6.4 floors); qualification never caps to 4–16 ops; smoke **withdraws** bottleneck claims incl. `io_queue_underdriven`; multi-rep real_store path remains; CLI `--class`; **board `in_review` (not qualification-accept)** | true controlled qualification run on controlled host (120s+512MiB floors + sustained window); principal accept after re-review | real-store campaign evidence |
