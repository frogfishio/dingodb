...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Prior accepted: aggregates≠plan; hashed preflight/env; multiproc; store hooks; plumbing. **This slice:** overhead legs share `execute_workload_puts` with measured cell — same **WorkloadContract** (duration/byte floors or smoke cap, distribution, durability, concurrency, outstanding, shards, batch, db_state, features, interference); **per-cell** `observer_overhead_reports` + hashed `observer_overhead.json`; attribution looks up by cell_id. Evidence: store-driver 188. **No 120s qual.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | overhead contract match + per-cell evidence |
