...
Updated: 2026-08-04 (**Compact Chimera architectural accept:** complete TPS ~12.4K→~37.9K (~3×); Chimera amp ~98%→~0.74%; burst ack ~43.8K; ~296MiB/s. **ETQ open.** **Next only:** ETQ-2 Single-Pass Enrichment Decode (`ETQ2_SINGLE_PASS_DECODE.md`) — one read/decode/`EnrichmentPlan`; ≥7 seg/s; slope≤0; exactly one decode pass; no workers first. Residual: enrich complete ~4.93 vs produce ~5.57 seg/s. **AWO paused.** Prior: ETQ-0 accept (Chimera 2× amp root cause). Prior 2026-08-03: **P0 DEF-SCAN-001** T7–T12 accepted labor, package not accepted. **AWO T11 evidence freeze principal `done`.** **AWO-Q series:** Feature `d0ae3c06` — Q0 **done**; **AWO-Q1 package principal `done`**; **AWO-Q2 labor `in_review`** (`0a043642`). Q3–Q4 still `backlog`. Explicit Q2 non-claims: package accept, default-on, thr floors, PQH diagnostic, sparse product bound, collector `select_plan` wiring. Plan `AWO_QUALIFICATION_SERIES.md`. T11 smoke = mechanism floor only. **Not** AWO product package accept / floors / default-on. Scratch exFAT residual remains. Other AWO measure cards still `in_review` as applicable; query spine / APB **in_review**)
