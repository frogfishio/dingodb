...
Updated: 2026-08-01 (APB-2/APP-3 **active** create/upsert/list_keys/replace/delete_with/**add** + receipt.version=event_id OCC; HAR-1 active; APB-1 G1–G6; APP-5/4 accept)
...
| APB-2 | active | 2026-08-01 | APB-1 | façade **create** / **upsert** / **list_keys** / **replace** / **delete_with** / **add** (`KeyProfile::RandomV1` → `residiuum-key-random-v1`); **`WriteReceipt.version` = establishing event_id** (OCC; server put receipt aligned); `Error::VersionConflict`/`NotFound`; `cargo test -p residiuum-sdk --test apb2_facade_mutations` **3/3** | store Key Atomic CAS, sortable key profiles, durability options remote, concurrent/crash matrix; **no package accept** | safe single-key mutation |
...
| APP-3 | active | 2026-08-01 | APP-2, HAR-4 | façade put/get/delete + history + indexes (APB-1) + APB-2 mutations (create/upsert/list_keys/replace/delete_with/add) + OCC version alignment; dual pack residual | APP-2 scoreboard lag; store CAS; remote durability; crash matrices; **no package accept** | typed data/history/index |
...
| 5 | **APP-4 → APP-5 → APB-7** | APP-4/5 **accept**; **APB-1 active** (G1–G6 dual matrix); still need APB-6/APP-6 for APB-7 path |
