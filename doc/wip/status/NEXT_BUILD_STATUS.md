...
Updated: 2026-08-01 (APB-2/APP-3 **active** façade create/upsert/list_keys; HAR-1 active; APB-1 G1–G6; APP-5/4 accept)
...
| APB-2 | active | 2026-08-01 | APB-1 | façade **create** (absent-only) / **upsert** (inserted flag) / **list_keys**; put remains upsert; `cargo test -p residiuum-sdk --test apb2_facade_mutations` **1/1** | replace/if_version/CAS, generated keys (`add`), durability options remote, concurrent/crash matrix; **no package accept** | safe single-key mutation |
...
| APP-3 | active | 2026-08-01 | APP-2, HAR-4 | façade put/get/delete + history + indexes (APB-1) + create/upsert/list_keys (APB-2 slice); dual pack | APP-2 scoreboard lag; replace/CAS; remote durability; crash matrices; **no package accept** | typed data/history/index |
