...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Accepted: multiproc; encoded_frame_len; no synth attribution; store hooks; fail-closed preflight; exact counters/histograms/digest/drop accounting. **This slice:** separate `BoundaryAggregateSummary` from lossless `PhysicalWritePlan` (drops → no plan/replay claim); matched probe-off/on `ObserverOverheadReport`; preflight+environment bound into hashed evidence (`preflight.json`/`environment.json`/`environment_hash.json`). Evidence: store-driver 183. **No 120s qual.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | plan vs aggregates + overhead + evidence bind |
