...
| APP-5 | active | 2026-08-01 | APP-4 | `rql_app_core` §9 surface: multi-where AND, project/order(+nulls), coverage/consistency, present/missing/is null, in/not in, starts_with/contains; multi-clause builder↔RQL plan_hash goldens; unit tests **8/8**; profile `rql-app-core-v1` | T3 corpus fixtures + reject matrix; T4 budget/fuzz; **not package accept** until exit | RQL Application Core |
