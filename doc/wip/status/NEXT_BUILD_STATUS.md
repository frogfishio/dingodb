...
Updated: 2026-08-03 (**P0 DEF-SCAN-001** T7–T12 accepted labor, package not accepted. **AWO T11 evidence freeze principal `done`.** **AWO-Q series:** Feature `d0ae3c06` — Q0 **done**; **AWO-Q1 package principal `done`** (Q1.1–Q1.3 correctness scope: concurrent admit, exactly-once, wait-outside-mutex, barriers, concurrent rotate, exact reopen set, seal receipt fix; evidence `AWO_Q1_PACKAGE_CLOSEOUT.md`). **Next: AWO-Q2 Adaptive decision quality `todo`** (`0a043642`); Q3–Q4 still `backlog`. Explicit Q1 non-claims: crash, sustained PQH, Adaptive quality, sparse bounds, default-on. Plan `AWO_QUALIFICATION_SERIES.md`. T11 smoke = mechanism floor only. **Not** AWO product package accept / floors / default-on. Scratch exFAT residual remains. Other AWO measure cards still `in_review` as applicable; query spine / APB **in_review**)
