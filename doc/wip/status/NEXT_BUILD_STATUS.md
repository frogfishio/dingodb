...
| PQH-11 | active | 2026-07-31 | PQH-10 | **Still not qualification-accept.** Accepted: multiproc; encoded_frame_len; no synth attribution; store hooks; fail-closed preflight. **This slice:** boundary probe no silent truncation — exact counters, latency histograms (write/sync/append), blake3 event-chain digest, explicit coverage/drop accounting; measures duration, requested/completed bytes, outcome, shard, file role at actual write/sync boundary. Evidence: store 158; store-driver 179. **No 120s qual.** Board `in_review` | controlled host `--class qualification` 120s+512MiB + sustained window; principal accept | bounded probe metrics |
