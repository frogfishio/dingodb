...
| APP-5 | active | 2026-08-01 | APP-4 | `rql_app_core` §9 surface + **corpus** `spec/app/v1/rql_app_core_corpus_v1.json` (7 accept / 10 reject); `cargo test -p residiuum-sdk --test app5_rql_app_core` **3/3**; unit tests **8/8**; plan_vectors `source_rql` hash lock; profile `rql-app-core-v1` | T4 budget/fuzz; T5 scoreboard accept only after exit; **not package accept** | RQL Application Core |
