...
|----|-----------|--------|---------------------|
| F1 | Framing layout + magics + field offsets match FORMAT_SPEC §4 | **Met (impl)** | `residiuum-format::frame`; `frame_codec` tests |
...
| F12 | External review of framing, integrity, limits, chunks, envelopes, conflict, recovery | **Open** | [SECURITY_AUDIT_PACKAGE.md](../security/SECURITY_AUDIT_PACKAGE.md) prepared; audit **not** completed |
| F13 | Canonical encodings inventory published | **Met (this cut)** | §3 below |
| F14 | Compatibility / support-window policy published | **Met (this cut)** | §4 below + `residiuum-format::compat` |
...
|-----------------|----------|------|
| FORMAT_SPEC §13 destructive corpus | `crates/residiuum-format/tests/section13_corpus.rs` | Required wire tests |
| Frame codec unit suite | `crates/residiuum-format/tests/frame_codec.rs` | Encode/decode edges |
| Segment + scan suite | `crates/residiuum-format/tests/segment_and_scan.rs` | Seal / salvage |
| Property / hostile tests | `crates/residiuum-format/tests/stage_def_091_properties.rs` | DEF-091 |
| HP-000 format vectors | `spec/heap/vectors-v1.json` + `emit_hp000_format_vectors` | Heap/format golden |
| Protocol RPC goldens | `crates/residiuum-sdk/tests/fixtures/protocol/` | Network (separate label) |
...
round-trips the same golden byte vectors without linking `residiuum-format`.
...
Source of truth: `residiuum_format::wire_compat_matrix()` / `wire_support_summary()`.
...
| Check | Command |
|-------|---------|
| Format unit + §13 corpus | `cargo test -p residiuum-format` |
| §13 only | `cargo test -p residiuum-format --test section13_corpus` |
| Compat / freeze guard | `cargo test -p residiuum-format compat` |
| DEF-091 properties | `cargo test -p residiuum-format --test stage_def_091_properties` |
| Fuzz property bar | `RESIDIUUM_FUZZ_SKIP_CARGO_FUZZ=1 ./scripts/fuzz-smoke.sh` |
| Migrate cut | `cargo test -p residiuum-store --features legacy-raw-store --test stage_def_052_migrate` |
| Freeze readiness API | `wire_freeze_readiness()` / `wire_is_frozen()` in `residiuum-format::compat` |
...
| Code guard | `residiuum_format::wire_is_frozen() == false` while draft |
