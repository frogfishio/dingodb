...
| HP-010 | Single-node qualification | **In progress (H3–H6 partial)** | Matrix stays `qualified=false`; Accept adds **support-bundle/health-detail confinement** and **executable §39 decide obligations**. H6 still needs full Verus + external review. |
...
| H3 Derived / operational coverage | **In progress** — HeapCap termination; single-owner admit; indexes/streams + query-escape; metrics/logs/export + **support-bundle/health-detail** confinement Accept. Named isolation profile registry still open. |
...
| H6 Isolation claim | **Partial** — Level 1 language; published limitations; TLA sketch + IsolationModel; **executable §39 decide obligations** (`authority_binding_holds`). Full Verus + external review still open — `may_advertise_qualified() == false`. |
...
Continue **HP-010** (close H6 full Verus + external review; named isolation
profile / metadata-hardened registry) until `qualified=true` is honest. In
...
NEXT   HP-010 single-node qualification  (H3 ops/health/bundle + §39 obligations; Verus/review open)
...
|------|---------|------------------------------|
| Full H-gate matrices, load/fuzz/restore/key-loss evidence | HP-010 | Support-bundle/health + §39 decide obligations Accept landed; H6 full Verus + external review still block `qualified=true` |
...
derived-path indexes/streams; query escape; metrics/logs/export + support-bundle
+ health-detail confinement; load/latency + structured fuzz budgets; operator
runbook; lifecycle crash-matrix (peer heaps unaffected); `HeapIsolation.tla` +
connected Rust `IsolationModel`; executable §39 decide obligations
(`authority_binding_holds` / `h6_decide_obligations`). Full Verus proofs and
