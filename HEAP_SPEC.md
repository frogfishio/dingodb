...
| HP-010 | Single-node qualification | **In progress (H3–H6 partial)** | Matrix `spec/heap/qualification/hp010-matrix-v1.json`; claim stays `qualified=false`; Accept adds **metrics/logs/export confinement**, **lifecycle crash-matrix** (peer heaps unaffected), **connected Rust IsolationModel ↔ TLA Inv**. H6 still needs full Verus + external review. |
...
| H3 Derived / operational coverage | **In progress** — HeapCap termination; single-owner admit; indexes/streams + query-escape; **metrics/logs/export confinement** Accept. Support-bundle / health-detail matrices still open. |
...
| H6 Isolation claim | **Partial** — Level 1 language; published limitations; TLA sketch + **connected Rust IsolationModel**; query-escape evidence. Full Verus + external review still open — `may_advertise_qualified() == false`. |
...
crates/dingo-heap/src/operational.rs     # metrics/logs/export confinement (H3)
crates/dingo-heap/src/isolation_model.rs # connected Rust ↔ TLA Inv (H6)
...
formal/heap/                   # TLA+ HeapIsolation sketch (+ MC cfg)
...
Continue **HP-010** (close H6 full Verus + external review, remaining H3
support-bundle/health-detail matrices) until `qualified=true` is honest. In
parallel: HP-006 physical rewrite, HP-008 §32.4 / default qualified listener,
HP-009 live filesystem tier wipe / HSM adapters.

#### Remaining delivery sequence

```text
DONE   HP-000 → … → HP-008 → HP-006(core) → HP-009(core+DR/key+media/retention)
NEXT   HP-010 single-node qualification  (ops/crash/connected-model landed; Verus/review open)
...
|------|---------|------------------------------|
| Full H-gate matrices, load/fuzz/restore/key-loss evidence | HP-010 | Ops + lifecycle crash + connected model Accept landed; H6 full Verus + external review still block `qualified=true` |
...
| Remote `connect_heap`, SubjectV2 put/get, holder signer export | HP-007 |
| Authority/RPC vector remainder in `spec/heap` | HP-000 |
| Verus/Kani connected proofs | HP-001 |
| Cut legacy Stages 3–9 off default `legacy-raw-store` | HP-003 / HP-006 |
...
as defined by [DX_SPEC.md](DX_SPEC.md).
...
Purge follows [DATABASE_DOCTRINE.md](DATABASE_DOCTRINE.md).
...
HP-000 activates only operations 1–3. Their schemas are exact:
...
envelope with only the Boolean result and never a reason. Every heap operation
in §32.3 remains reserved at HP-000. HP-007, HP-008, or HP-009 may activate an
operation only by first adding its exact table/schema amendment to this
contract. Thus no developer is asked or permitted to invent an unspecified
`args` or `result` shape.
...
same operation ID either commits the byte-identical event or returns its
existing receipt. It never manufactures a different event from the changed
head.

A serving node authorizes a heap only while it holds a committed lease matching
that heap's resident ID, epoch, security revision, chain-head hash, and current
control term, has applied at least `min_control_index`, and trusted time is
before `not_after_unix_ms`. A lease is non-transferable and one heap only. The
default and maximum lease duration is 5 seconds. The qualified cluster profile
requires maximum clock uncertainty of 250 ms. Loss of quorum therefore stops
new protected requests within 5.25 seconds.

A security barrier—master cycle, heap suspension, purge, recovery-policy
change—becomes externally effective after:

1. its mutation is committed;
2. every new lease requires an applied index at or beyond that mutation; and
3. the last lease issued before the mutation has expired.

The control leader returns a `barrier_receipt` containing deployment ID, heap
ID, authority epoch, resulting security revision, committed index, term, last
old-lease expiry, and authority-event hash. Administrative tooling must not
report completion before this receipt's expiry is reached.

Data groups store one heap per replicated segment and include `HeapId` in every
Raft command. A replica applies a command only when command heap, partition
assignment, local segment owner, and authenticated peer placement capability
all agree. Cluster peers use a separate mTLS identity containing
`DeploymentId` and `NodeId`; a client HeapKey is never a peer credential.

Restoring payload data cannot create or replace the security-control group.
Restoring that group is a separately authorized disaster-recovery ceremony
which advances `AuthorityEpoch` and invalidates every previously issued key.

## 38. Cryptographic conformance vectors

The repository MUST contain these values in
`spec/heap/vectors-v1.json`. CI decodes, re-encodes, signs, verifies, and
compares every byte. Hex is lowercase without separators.

Inputs:

```text
master_seed = 1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100
master_public_key = 712651f450ba05b63898b99ef5f7ba45632e8e2527f7f715cd671ec4024cc51e
holder_seed = 000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f
holder_public_key = 03a107bff3ce10be1d70dd18e74bc09967e4d6309ba50d5f1ddc8664125531b8
deployment_id = 000102030405460788090a0b0c0d0e0f
heap_id = 101112131415461798191a1b1c1d1e1f
certificate_id = 2021222324254627a8292a2b2c2d2e2f
proof_id = 3031323334354637b8393a3b3c3d3e3f
issuer_key_hash = 141ddf2e77d4f690748cf74ecd390d44687d477b31b8931fa37abd02c35dbaba
```

Certificate:

```text
protected = a2012703781f6170706c69636174696f6e2f64696e676f2e686561702d6b65792b63626f72
payload = 
sig_structure = 
cose_sign1 = 
sha256 = a9ed91fc5bbb1db6500317336be6c3401e9698a16263a72a94be360395384057
```

Holder proof:

```text
protected = a201270378216170706c69636174696f6e2f64696e676f2e686561702d70726f6f662b63626f72
payload = 
sig_structure = 
cose_sign1 = 
```

At minimum, negative vectors mutate each protected header, payload field,
signature byte, UUID version/variant, time boundary, nonce, TLS exporter,
rights bit, critical constraint, and blacklist entry. Every malformed or
non-canonical input must return one stable public `heap_unavailable` result and
a specific internal diagnostic code.

### 38.1 Complete vector corpus

The certificate/proof bytes above are the bootstrap vectors, not the complete
corpus. HP-000 is not accepted until `vectors-v1.json` also contains exact
encoded bytes, hashes, and expected decisions for:

- issuance request;
- creation root, hard cycle, graceful cycle, root recovery, and
  recovery-policy replacement;
- all four master-mutation kinds and public/internal operational events;
- authority head, access policy, event wrapper, time-floor slot, anchor
  comparison, control-barrier requests/responses, and genesis publication;
- every SubjectV2 subtype, all descriptor kinds, descriptor chains, and
  §34.7 hashes;
- common RPC envelopes, request-binding hashes, operation-ID replay/conflict,
  and each active operation schema;
- every boundary value immediately below, at, and above a stated numeric or
  size limit.

Corpus IDs, UUIDs, timestamps, counters, seeds, names, and input bytes are
literal fields in the JSON artifact, never implicit generator defaults. Two
independent implementations using different CBOR/COSE libraries must produce
the same accepted bytes and decisions. The production encoder cannot generate
its own expected output during the test. For each accepted object, the corpus
contains mutations for missing, duplicate, reordered/non-canonical, unknown,
wrong-type, wrong-length, wrong-owner, wrong-domain, and trailing data as
applicable.

## 39. Verification implementation

The verification stack is part of the feature, not optional documentation:

- [TLA+/TLC](https://lamport.azurewebsites.net/tla/tools.html) models state
  transitions, rotation/grace, leases, restore, purge, and crash recovery in
  `formal/heap/`;
- [Verus](https://github.com/verus-lang/verus) proves the pure isolation and
  authority-decision kernel in `verification/heap-verus/`;
- [Kani](https://model-checking.github.io/kani/) checks bounded parsers,
  subject decoding, rights/constraint intersection, and recovery state
  machines;
- property tests compare optimized runtime decisions against the pure model;
- fuzzers cover every network, CBOR, frame, catalog, recovery, and migration
  decoder.
...
formal/heap/HeapCluster.tla
formal/heap/HeapRecovery.tla
formal/heap/MC*.cfg
verification/heap-verus/src/lib.rs
scripts/verify-heap.sh
```

`HeapIsolation.tla` represents bytes as independently damageable units and
checks that every surviving admitted unit has exactly one owner.
`HeapAuthority.tla` checks generation, blacklist, grace, time floor, and
terminal states. `HeapCluster.tla` explores partitions, leader changes, stale
leases, and barriers. `HeapRecovery.tla` explores crashes between each durable
write/rename/sync step.

The executable Rust decision function is total:
...
It performs no I/O, allocation after snapshot construction, ambient clock
read, string lookup, or global-state access. The server calls it exactly once
at admission. The Verus model proves:

1. allow implies certificate heap equals snapshot heap;
2. allow implies requested immutable object is inside every applicable
   constraint;
3. unknown rights or critical constraints cannot allow;
4. a terminal state cannot allow;
5. epoch/generation acceptance follows the frozen grace rule.

CI runs `scripts/verify-heap.sh quick` on each change and `full` nightly and
before a qualified release. A release cannot waive a failed invariant; it can
only ship under a different, explicitly unqualified profile.

## 40. Ordered development work packages

The following order is normative. A later package may prototype, but it cannot
merge into the qualified path before its dependencies.
...
- `spec/heap/operations-v1.json`;
- common request/response schemas and exact schemas/fixtures for active
  operations 1–3;
- `spec/heap/cbor-v1.json`;
- `spec/heap/vectors-v1.json`;
- generated Rust operation/right constants;
- a checker proving the generated tables equal this specification.

Accept when every §32.3 ID appears exactly once with its frozen status and
rights, only 1–3 are active, schemas exist for every active entry and none is
silently callable, two independent decoders reproduce every vector
byte-for-byte, and the negative corpus is rejected.
...
Accept when no public constructor can forge `HeapCap`, rights intersection is
property-tested, and the pure kernel passes Verus/Kani targets.
...
Accept when a corpus with holes, truncation, swapped paths, conflicting labels,
...
Accept when public Rustdoc contains no unscoped scan/mutate primitive and a
compile-fail suite proves callers cannot construct or substitute a heap ID
after obtaining a capability.
...
heap; HP-005 binds genesis to authority.

Accept when deleting every catalog and index reconstructs the same names,
aliases, IDs, and owner heap from surviving descriptors.

### HP-005 — Authority and local ceremony

**Status:** landed in-tree (Accept core). See Implementation progress.

Depends on HP-001 and HP-004. Deliver two-slot authority store, trusted time
floor, the separate `dingo-authority` executable, master-key provider,
local-only issue/cycle/recovery commands, the fixed data-server
security-barrier/reload client, staged-genesis protocol, receipts,
root/transition/mutation/operational event chain, resident access policy,
blacklist, and graceful rotation.

Accept when crash injection proves atomicity, rollback/fork tests fail closed,
the data-service binary has no `dingo-authority` dependency or concrete master
provider, reload cannot mutate authority, every genesis crash boundary
converges according to §8.9.1, and no network-callable dependency path reaches
signing or destruction.

### HP-006 — Legacy migration

Depends on HP-004 and HP-005. Deliver all phases in section 36, resume logic,
inventory verification, preflight/rollback tooling, and operator report.

Accept when injected crashes at every durable step converge to one result with
no duplicate/lost admitted frame and phase 6 cannot occur with an unlabelled
active frame.
...
Accept when two heap handles with identical collection names cannot exchange
typed handles, cursors, batch members, or pooled connections.

### HP-008 — Qualified network protocol

Depends on HP-005 and HP-007. Deliver TLS 1.3, challenge/proof/welcome,
uniform rejection, request registry, admission limits, audit diagnostics, and
removal of token/RBAC from the qualified listener. Before activating any heap
operation, land its exact §32.4 schema amendment, fixtures, resource bounds,
and public error mapping; an implementation patch cannot define these only in
code.

Accept when packet captures reveal neither distinguishable heap existence nor
authorization failure causes, proof replay fails, and the request hot path
performs zero authority-store lookups.

### HP-009 — Lifecycle, backup, and recovery

Depends on HP-004 and HP-005. Deliver suspension/retirement/purge, retention,
holds, data-key destruction, heap-aware backup manifests, restore-to-new-epoch,
and recovery tooling. Any remotely exposed lifecycle operation follows the
same §32.4 activation rule.

Accept when payload-only restore cannot grant access, purge has a verifiable
receipt, and damage tests demonstrate that healthy independently owned units
remain readable.

### HP-010 — Single-node qualification

**Implementation status (2026-07-29):** evidence harness + H3–H5 Accept drills +
H6 partial — matrix + claim surface (`qualified=false`); differential NI; HeapCap
termination; single-owner admit; key-loss; restore; retention/incomplete-purge;
derived-path indexes/streams; query escape; metrics/logs/export confinement;
load/latency + structured fuzz budgets; operator runbook; lifecycle crash-matrix
(peer heaps unaffected); `HeapIsolation.tla` + connected Rust `IsolationModel`.
Full Verus proofs and external security review remain open.
...
Accept only when all mandatory matrices in this specification have recorded,
reproducible evidence. This is the first release allowed to advertise the
`dingo-heap-v1` qualified profile.

### HP-011 — Cluster control and placement

Depends on HP-010. Deliver the security-control Raft group, committed leases,
barrier receipts, peer identity, placement capabilities, replica owner checks,
and separate authority disaster recovery.

Accept when model checking and partition tests prove stale nodes stop serving
within the lease bound and no payload restore or placement error can change
heap ownership or authority.

### HP-012 — Cluster qualification

Depends on HP-011. Execute the full cluster fault matrix: partitions, clock
skew, delayed/reordered messages, leader churn, disk rollback, partial restore,
node theft, mixed-version rejection, and concurrent authority barriers.

Accept only with the same evidence rules as HP-010. Cluster support before this
package is experimental and MUST report `qualified=false`.

## 41. Definition of ready for implementation

This v0.9 document authorized HP-000 to begin immediately. As of the
Implementation progress date above, HP-000…HP-009 have in-tree deliveries with
the gaps recorded there; HP-010 is the next qualification package (with
HP-006/008/009 residuals still required for full evidence). Later packages
still MUST NOT merge into the
...
authorization surface, or cluster lease behavior inside an implementation
patch. A required deviation is first proposed as a numbered change to this
specification, including compatibility, threat, migration, proof, and test
consequences.
...
machine-readable registry or vector changes
new positive and negative tests
fault-injection points added
formal-model impact
legacy compatibility impact
remaining package exit conditions
```

Review rejects:

- code that introduces a second source of operation, rights, state, or wire
  truth;
- public raw-store access added “temporarily”;
- qualified behavior hidden behind an undocumented fallback;
- an implementation-only field or state transition;
- tests that prove only ordinary success paths;
- a generated artifact changed without changing and reviewing its source;
- a claim-level change without updated qualification evidence.

The implementation is complete only when code, generated contract artifacts,
formal models, negative corpora, operational procedures, and qualification
evidence agree. Passing ordinary functional tests alone is not completion.
...
3. transcribe every §32 allocation, right, state admission, and status into
   `operations-v1.json`, with only IDs 1–3 active;
4. transcribe and independently verify §38 in `vectors-v1.json`;
5. generate Rust constants and exhaustive registry tests;
6. add a checker that compares generated tables with committed artifacts;
7. add the common envelope schemas plus one accepted and one rejected
   request/response fixture for each active operation;
8. make CI reject drift, duplicate identifiers, missing schemas, unknown
   rights, incomplete state coverage, or dispatch of a reserved operation.

HP-000 changes no storage behavior and exposes no new network listener. Its
review establishes the single machine-readable contract consumed by HP-001
and every later package.

### 41.2 Qualification boundary

The first product release may qualify `single-node` after HP-010 and H0–H6
pass for that profile. It does not wait for cluster work and makes no cluster
claim.

The `cluster` profile remains experimental until HP-011, HP-012, HC1, and H6
all pass with cluster-specific evidence. A cluster prototype cannot weaken or
delay the single-node isolation kernel.
