...
| **HP-007** | SDK capability surface | **Landed (Accept isolation + SubjectV2 data path)** | `dingo-sdk::heap`: `DingoDeployment`, `Heap`, `HeapCollection`/`HeapStream`, heap-bound pool, `SignedCursor`, batch membership checks. **SubjectV2 put/get/delete** on `HeapCollection` via `HeapStore` (binary subjects; foreign-heap SubjectV2 reject; legacy v1 subjects rejected on qualified façade). Accept: identical collection names cannot exchange handles/cursors/batch members/pooled connections **and** same app keys stay isolated under SubjectV2. **Gaps:** remote `connect_heap`, `dangerous-key-export` holder signer. |
...
| HP-010 | Single-node qualification | **In progress (H3 Accept; H6 partial)** | Matrix stays `qualified=false`. **H3 Accept** (derived/ops + profiles + metadata-hardened). Complete-path review + external security review brief + `pure_proofs` bundle landed. **H1 advanced:** SubjectV2 put/get Accept on heap SDK. H0/H1/H2/H4/H5 still partial (legacy flat SDK / CPR-001, remote `connect_heap`, reserved ops, recovery residuals). H6 still needs machine-checked Verus/Kani + **signed** external review + CPR residual close. |
...
| H1 Heap-bound SDK | **In progress** — HP-007 typed handles + SubjectV2 put/get Accept; remote `connect_heap` still open; default flat SDK still CPR-001. |
...
| Remote `connect_heap`, holder signer export (`dangerous-key-export`) | HP-007 residual (SubjectV2 put/get Accept landed) |
