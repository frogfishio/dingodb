...
| **HP-007** | SDK capability surface | **Landed (Accept isolation + SubjectV2 + connect_heap process ops)** | `dingo-sdk::heap`: `DingoDeployment`, `Heap`, `HeapCollection`/`HeapStream`, heap-bound pool, `SignedCursor`, batch membership checks. **SubjectV2 put/get/delete** on `HeapCollection`. **Remote** `Dingo::connect_heap` + `HeapCredential`/`HolderSigner`/`RemoteHeapOptions`/`RemoteHeap` (TLS + HeapKey welcome; process ops 1–3). Feature `dangerous-key-export` gates reference `InMemoryHolderKey`. Accept: isolation + SubjectV2 data + connect_heap welcome/ping/name-reject. **Gaps:** remote collection data ops (§32.4), CPR-001 flat default. |
...
| H1 Heap-bound SDK | **In progress** — HP-007 typed handles + SubjectV2 put/get + remote `connect_heap` process-ops Accept; remote data ops §32.4 still open; default flat SDK still CPR-001. |
...
| Remote collection data ops on `RemoteHeap` (§32.4 schemas first) | HP-007 residual (`connect_heap` process ops Accept landed) |
